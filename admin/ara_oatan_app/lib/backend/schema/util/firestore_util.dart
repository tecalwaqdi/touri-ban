...
/// يقبل [DocumentReference] أو مسار Firestore كنص (بيانات قديمة/يدوية).
DocumentReference? castDocRef(dynamic value) {
  ...
  if (value is Map) {
    final nested = value['reference'] ??
        value[r'$ref'] ??
        value['path'] ??
        value['referenceValue'];
    if (nested is String) {
      return castDocRef(nested);
    }
  }
  return null;
}

List<DocumentReference>? castDocRefList(dynamic value) {
  if (value is! List) return null;
  final refs = value.map(castDocRef).whereType<DocumentReference>().toList();
  return refs;
}
