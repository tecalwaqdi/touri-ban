...
class _FlutterFlowGoogleMapState extends State<FlutterFlowGoogleMap> {
  ...
  LatLng get initialPosition {
    final raw = widget.initialLocation;
    // Neutral shell only — never invent Makkah/Saudi as a global default.
    if (raw == null || (raw.latitude == 0 && raw.longitude == 0)) {
      return const LatLng(20.0, 0.0);
    }
    if (raw.latitude.isNaN ||
        raw.longitude.isNaN ||
        raw.latitude.isInfinite ||
        raw.longitude.isInfinite) {
      return const LatLng(20.0, 0.0);
    }
    return raw.toGoogleMaps();
  }
