...
/**
 * Exact Driver App open-pool query (DriverOrderMatch.queryBuilder):
 *   collection order
 *   where status_code == pending_driver
 *   where ALLNOW == true
 *   orderBy data_order DESC
 * Used by Now /neworder ("الطلبات الجديدة" / المتاحة).
 */
function driverAvailableOrdersQuery(db) {
  const {
    collection,
    query,
    where,
...
describe("Driver available-order LIST (Now / neworder parity)", () => {
  const openOrderId = "open-pool-order-1";
  const assignedOrderId = "assigned-order-1";
  const saudiPath = "countries/saudi_arabia";

  async function seedOpenPool() {
    const seedDb = testEnv.authenticatedContext("seed-paths").firestore();
    const saudi = doc(seedDb, "countries", "saudi_arabia");
    const driverRef = doc(seedDb, "user", "drv-approved");
    await seed({
      "countries/saudi_arabia": {currency_code: "SAR", acctev: true},
      "user/drv-approved": {
        ismndob: true,
        actev_mndob: true,
        registration_status: "approved",
      ...
      // Production mismatch: admin-activated + still pending_review (live case).
      "user/drv-legacy-pending": {
        ismndob: true,
        actev_mndob: true,
      ...
      [`order/${openOrderId}`]: {
        USER: doc(seedDb, "user", "customer-pool"),
        Rev_dolh: saudi,
        status_code: "pending_driver",
        ALLNOW: true,
        mndob_user: null,
    ...
    return {saudiPath, driverRef};
  }

  it("A_approved_active_driver_lists_available_orders ALLOW", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("drv-approved").firestore();
    await assertSucceeds(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("H_legacy_pending_review_with_actev_lists_available_orders ALLOW", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("drv-legacy-pending").firestore();
    await assertSucceeds(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("B_non_driver_cannot_list_available_orders DENY", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("plain-user").firestore();
    await assertFails(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("C_inactive_driver_lists_available_orders DENY", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("drv-inactive").firestore();
    await assertFails(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("C2_rejected_driver_lists_available_orders DENY", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("drv-rejected").firestore();
    await assertFails(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("D_customer_cannot_use_driver_pool_query DENY", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("customer-pool").firestore();
    await assertFails(getDocs(driverAvailableOrdersQuery(db)));
  });

  it("E_driver_cannot_write_arbitrary_order_fields DENY", async () => {
    await seedOpenPool();
    const db = testEnv.authenticatedContext("drv-approved").firestore();
    await assertFails(updateDoc(doc(db, "order", openOrderId), {
      total: 1,
      amount_halalas: 1,
    }));
  });

  it("F_assigned_order_get_still_ALLOW", async () => {
    await seedOpenPool();
    const db = testEnv.authenticatedContext("drv-approved").firestore();
    await assertSucceeds(getDoc(doc(db, "order", assignedOrderId)));
  });

  it("G_country_field_present_on_open_order_still_listable", async () => {
    await seedOpenPool();
    const {getDocs} = require("firebase/firestore");
    const db = testEnv.authenticatedContext("drv-approved").firestore();
    const snap = await assertSucceeds(getDocs(driverAvailableOrdersQuery(db)));
    // Emulator returns matching docs the rules allow; open pool has Rev_dolh set.
    const ids = snap.docs.map((d) => d.id);
    if (!ids.includes(openOrderId)) {
      throw new Error(`expected ${openOrderId} in ${JSON.stringify(ids)}`);
    }
  });
});
