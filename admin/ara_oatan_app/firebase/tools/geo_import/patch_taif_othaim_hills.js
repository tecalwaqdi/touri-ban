...
/**
 * Patch Taif Othaim Hills: user photo + rename Othaim Mall → Othaim Hills.
 *
 * Usage:
 *   node patch_taif_othaim_hills.js           # dry-run
 *   node patch_taif_othaim_hills.js --apply
 */
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'toury-taif-othaim-'));
...
const ID = 'lm_sa_taif_othaim-mall-taif';
const FILE = 'taif_othaim_hills.png';
const NAMES = {
  ar: 'العثيم هليز',
  en: 'Othaim Hills',
  ru: 'Отайм Хиллз',
...
async function main() {
  ...
  const patch = {
    img1: dataUrl,
    img_source: 'user_photo_IMG_6978',
  ...
  const out = path.join(REPORTS, 'taif_othaim_hills_patch.json');
