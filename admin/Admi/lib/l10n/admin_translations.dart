/// Shared admin-panel strings (all supported app languages).
/// Merged into [FFLocalizations.getText] lookup.
const kAdminTranslations = <String, Map<String, String>>{
  'adm_app_title': {
    'en': 'Control Panel',
    'pt': 'Painel de controle',
    'fr': 'Panneau de contrôle',
    'ar': 'لوحة التحكم',
    'zh_Hans': '控制面板',
    'tr': 'Kontrol Paneli',
    'ur': 'کنٹرول پینل',
    'ru': 'Панель управления',
    'ky': 'Башкаруу панели',
    'az': 'İdarəetmə paneli',
    'ka': 'მართვის პანელი',
  },
  'adm_splash_title': {
    'en': 'Touri Taxi',
    'pt': 'Touri Taxi',
    'fr': 'Touri Taxi',
    'ar': 'توري تاكسي',
    'zh_Hans': 'Touri Taxi',
    'tr': 'Touri Taxi',
    'ur': 'Touri Taxi',
    'ru': 'Touri Taxi',
    'ky': 'Touri Taxi',
    'az': 'Touri Taxi',
    'ka': 'Touri Taxi',
  },
  'adm_splash_tagline': {
    'en': 'The First Global Saudi Tourism App',
    'pt': 'O primeiro app global de turismo saudita',
    'fr': 'La première application touristique saoudienne mondiale',
    'ar': 'أول تطبيق سعودي سياحي عالمي',
    'zh_Hans': '首个全球沙特旅游应用',
    'tr': 'İlk Küresel Suudi Turizm Uygulaması',
    'ur': 'پہلی عالمی سعودی سیاحتی ایپ',
    'ru': 'Первое глобальное саудовское туристическое приложение',
    'ky': 'Бirinchi global saudiya turizm kolduk',
    'az': 'İlk qlobal Səud turizm tətbiqi',
    'ka': 'პირველი გლობალური საუდის ტურისტული აპი',
  },
  'adm_theme_title': {
    'en': 'Appearance',
    'pt': 'Aparência',
    'fr': 'Apparence',
    'ar': 'المظهر',
    'zh_Hans': '外观',
    'tr': 'Görünüm',
    'ur': 'ظاہری شکل',
    'ru': 'Оформление',
    'ky': 'Көрүнүш',
    'az': 'Görünüş',
    'ka': 'გარეგნობა',
  },
  'adm_theme_subtitle': {
    'en': 'Switch between light and dark mode for the control panel.',
    'pt': 'Alterne entre os modos claro e escuro do painel de controle.',
    'fr':
        'Basculez entre le mode clair et le mode sombre pour le panneau de contrôle.',
    'ar': 'بدّل بين الوضع العادي والليلي للوحة التحكم.',
    'zh_Hans': '在控制面板的浅色与深色模式之间切换。',
    'tr': 'Kontrol paneli için açık ve koyu mod arasında geçiş yapın.',
    'ur': 'کنٹرول پینل کے لیے لائٹ اور ڈارک موڈ تبدیل کریں۔',
    'ru': 'Переключайте светлый и тёмный режим панели управления.',
    'ky': 'Башкаруу панели үчүн жарык жана караңгы режимди которуңуз.',
    'az': 'İdarəetmə paneli üçün açıq və tünd rejim arasında keçid edin.',
    'ka': 'გადართეთ მართვის პანელის ნათელი და მუქი რეჟიმი.',
  },
  'adm_theme_light': {
    'en': 'Light mode',
    'pt': 'Modo claro',
    'fr': 'Mode clair',
    'ar': 'الوضع العادي',
    'zh_Hans': '浅色模式',
    'tr': 'Açık mod',
    'ur': 'لائٹ موڈ',
    'ru': 'Светлый режим',
    'ky': 'Жарык режим',
    'az': 'Açıq rejim',
    'ka': 'ნათელი რეჟიმი',
  },
  'adm_theme_dark': {
    'en': 'Dark mode',
    'pt': 'Modo escuro',
    'fr': 'Mode sombre',
    'ar': 'الوضع الليلي',
    'zh_Hans': '深色模式',
    'tr': 'Koyu mod',
    'ur': 'ڈارک موڈ',
    'ru': 'Тёмный режим',
    'ky': 'Караңгы режим',
    'az': 'Tünd rejim',
    'ka': 'მუქი რეჟიმი',
  },

  // Actions
  'adm_cancel': {
    'en': 'Cancel',
    'pt': 'Cancelar',
    'fr': 'Annuler',
    'ar': 'إلغاء',
    'zh_Hans': '取消',
    'tr': 'İptal',
    'ur': 'منسوخ',
    'ru': 'Отмена',
    'ky': 'Жокко чыгаруу',
    'az': 'Ləğv et',
    'ka': 'გაუქმება',
  },
  'adm_no': {
    'en': 'No',
    'pt': 'Não',
    'fr': 'Non',
    'ar': 'لا',
    'zh_Hans': '否',
    'tr': 'Hayır',
    'ur': 'نہیں',
    'ru': 'Нет',
    'ky': 'Жок',
    'az': 'Xeyr',
    'ka': 'არა',
  },
  'adm_yes_delete': {
    'en': 'Yes, delete',
    'pt': 'Sim, delete',
    'fr': 'Oui, delete',
    'ar': 'نعم، احذف',
    'zh_Hans': '是，删除',
    'tr': 'Evet, sil',
    'ur': 'ہاں، حذف کریں',
    'ru': 'Да, удалить',
    'ky': 'Ооба, өчүрүү',
    'az': 'Bəli, sil',
    'ka': 'დიახ, წაშლა',
  },
  'adm_delete': {
    'en': 'Delete',
    'pt': 'Excluir',
    'fr': 'Supprimer',
    'ar': 'حذف',
    'zh_Hans': '删除',
    'tr': 'Sil',
    'ur': 'حذف',
    'ru': 'Удалить',
    'ky': 'Өчүрүү',
    'az': 'Sil',
    'ka': 'წაშლა',
  },
  'adm_confirm': {
    'en': 'Confirm',
    'pt': 'Confirmar',
    'fr': 'Confirmer',
    'ar': 'تأكيد',
    'zh_Hans': '确认',
    'tr': 'Onayla',
    'ur': 'تصدیق',
    'ru': 'Подтвердить',
    'ky': 'Ырастоо',
    'az': 'Təsdiqlə',
    'ka': 'დადასტურება',
  },
  'adm_ok': {
    'en': 'OK',
    'pt': 'OK',
    'fr': 'OK',
    'ar': 'حسناً',
    'zh_Hans': '好的',
    'tr': 'Tamam',
    'ur': 'ٹھیک ہے',
    'ru': 'ОК',
    'ky': 'Макул',
    'az': 'Oldu',
    'ka': 'კარგი',
  },
  'adm_retry': {
    'en': 'Retry',
    'pt': 'Tentar novamente',
    'fr': 'Réessayer',
    'ar': 'إعادة المحاولة',
    'zh_Hans': '重试',
    'tr': 'Tekrar dene',
    'ur': 'دوبارہ کوشش کریں',
    'ru': 'Повторить',
    'ky': 'Кайталоо',
    'az': 'Yenidən cəhd et',
    'ka': 'თავიდან ცდა',
  },
  'adm_refresh': {
    'en': 'Refresh',
    'pt': 'Atualizar',
    'fr': 'Actualiser',
    'ar': 'تحديث',
    'zh_Hans': '刷新',
    'tr': 'Yenile',
    'ur': 'تازہ کریں',
    'ru': 'Обновить',
    'ky': 'Жаңылоо',
    'az': 'Yenilə',
    'ka': 'განახლება',
  },
  'adm_load_more': {
    'en': 'Load more',
    'pt': 'Carregar mais',
    'fr': 'Charger plus',
    'ar': 'تحميل المزيد',
    'zh_Hans': '加载更多',
    'tr': 'Daha fazla yükle',
    'ur': 'مزید لوڈ کریں',
    'ru': 'Загрузить ещё',
    'ky': 'Дагы жүктөө',
    'az': 'Daha çox yüklə',
    'ka': 'მეტის ჩატვირთვა',
  },
  'adm_save_edits': {
    'en': 'Save changes',
    'pt': 'Salvar changes',
    'fr': 'Enregistrer changes',
    'ar': 'حفظ التعديلات',
    'zh_Hans': '保存更改',
    'tr': 'Değişiklikleri kaydet',
    'ur': 'تبدیلیاں محفوظ کریں',
    'ru': 'Сохранить изменения',
    'ky': 'Өзгөртүүлөрдү сактоо',
    'az': 'Dəyişiklikləri saxla',
    'ka': 'ცვლილებების შენახვა',
  },
  'adm_saving': {
    'en': 'Saving...',
    'pt': 'Salvando...',
    'fr': 'Enregistrement...',
    'ar': 'جاري الحفظ...',
    'zh_Hans': '正在保存...',
    'tr': 'Kaydediliyor...',
    'ur': 'محفوظ ہو رہا ہے...',
    'ru': 'Сохранение...',
    'ky': 'Сакталууда...',
    'az': 'Saxlanılır...',
    'ka': 'ინახება...',
  },

  // CRUD success
  'adm_added_success': {
    'en': 'Added successfully',
    'pt': 'Adicionared successfully',
    'fr': 'Ajoutered successfully',
    'ar': 'تمت الإضافة بنجاح',
    'zh_Hans': '添加成功',
    'tr': 'Başarıyla eklendi',
    'ur': 'کامیابی سے شامل ہو گیا',
    'ru': 'Успешно добавлено',
    'ky': 'Ийгиликтүү кошулду',
    'az': 'Uğurla əlavə edildi',
    'ka': 'წარმატებით დაემატა',
  },
  'adm_saved_success': {
    'en': 'Changes saved successfully',
    'pt': 'Alterações salvas com sucesso',
    'fr': 'Modifications enregistrées avec succès',
    'ar': 'تم حفظ التعديلات بنجاح',
    'zh_Hans': '更改已保存',
    'tr': 'Değişiklikler kaydedildi',
    'ur': 'تبدیلیاں کامیابی سے محفوظ ہو گئیں',
    'ru': 'Изменения сохранены',
    'ky': 'Өзгөртүүлөр сакталды',
    'az': 'Dəyişikliklər saxlanıldı',
    'ka': 'ცვლილებები შენახულია',
  },
  'adm_deleted_success': {
    'en': 'Deleted successfully',
    'pt': 'Excluird successfully',
    'fr': 'Supprimerd successfully',
    'ar': 'تم الحذف بنجاح',
    'zh_Hans': '删除成功',
    'tr': 'Başarıyla silindi',
    'ur': 'کامیابی سے حذف ہو گیا',
    'ru': 'Успешно удалено',
    'ky': 'Ийгиликтүү өчүрүлдү',
    'az': 'Uğurla silindi',
    'ka': 'წარმატებით წაიშალა',
  },
  'adm_image_selected': {
    'en': 'Image selected successfully',
    'pt': 'Imagem selecionada com sucesso',
    'fr': 'Image sélectionnée avec succès',
    'ar': 'تم اختيار الصورة بنجاح',
    'zh_Hans': '图片已选择',
    'tr': 'Görsel başarıyla seçildi',
    'ur': 'تصویر کامیابی سے منتخب ہو گئی',
    'ru': 'Изображение выбрано',
    'ky': 'Сүрөт тандалды',
    'az': 'Şəkil seçildi',
    'ka': 'სურათი არჩეულია',
  },

  // CRUD errors
  'adm_save_failed': {
    'en': 'Could not save',
    'pt': 'Não foi possível salvar',
    'fr': 'Échec de l\'enregistrement',
    'ar': 'تعذر الحفظ',
    'zh_Hans': '无法保存',
    'tr': 'Kaydedilemedi',
    'ur': 'محفوظ نہیں ہو سکا',
    'ru': 'Не удалось сохранить',
    'ky': 'Сакталган жок',
    'az': 'Saxlamaq mümkün olmadı',
    'ka': 'შენახვა ვერ მოხერხდა',
  },
  'adm_delete_failed': {
    'en': 'Could not delete',
    'pt': 'Não foi possível excluir',
    'fr': 'Échec de la suppression',
    'ar': 'تعذر الحذف',
    'zh_Hans': '无法删除',
    'tr': 'Silinemedi',
    'ur': 'حذف نہیں ہو سکا',
    'ru': 'Не удалось удалить',
    'ky': 'Өчүрүлгөн жок',
    'az': 'Silmək mümkün olmadı',
    'ka': 'წაშლა ვერ მოხერხდა',
  },
  'adm_update_failed': {
    'en': 'Could not update',
    'pt': 'Não foi possível atualizar',
    'fr': 'Échec de la mise à jour',
    'ar': 'تعذر التحديث',
    'zh_Hans': '无法更新',
    'tr': 'Güncellenemedi',
    'ur': 'اپ ڈیٹ نہیں ہو سکا',
    'ru': 'Не удалось обновить',
    'ky': 'Жаңыланган жок',
    'az': 'Yeniləmək mümkün olmadı',
    'ka': 'განახლება ვერ მოხერხდა',
  },
  'adm_upload_failed': {
    'en': 'Could not upload image',
    'pt': 'Não foi possível enviar a imagem',
    'fr': 'Impossible de téléverser l\'image',
    'ar': 'تعذر رفع الصورة',
    'zh_Hans': '无法上传图片',
    'tr': 'Görsel yüklenemedi',
    'ur': 'تصویر اپ لوڈ نہیں ہو سکی',
    'ru': 'Не удалось загрузить изображение',
    'ky': 'Сүрөт жүктөлгөн жок',
    'az': 'Şəkil yüklənmədi',
    'ka': 'სურათის ატვირთვა ვერ მოხერხდა',
  },

  // Delete dialogs
  'adm_delete_confirm_title': {
    'en': 'Confirm deletion',
    'pt': 'Confirmar deletion',
    'fr': 'Confirmer deletion',
    'ar': 'تأكيد الحذف',
    'zh_Hans': '确认删除',
    'tr': 'Silmeyi onayla',
    'ur': 'حذف کی تصدیق',
    'ru': 'Подтвердите удаление',
    'ky': 'Өчүрүүнү ырастоо',
    'az': 'Silməni təsdiqlə',
    'ka': 'წაშლის დადასტურება',
  },
  'adm_delete_confirm_body': {
    'en': 'Are you sure you want to delete "%s"?',
    'pt': 'Are you sure you want to delete "%s"?',
    'fr': 'Are you sure you want to delete "%s"?',
    'ar': 'هل أنت متأكد من حذف "%s"؟',
    'zh_Hans': '确定要删除“%s”吗？',
    'tr': '"%s" öğesini silmek istediğinizden emin misiniz?',
    'ur': 'کیا آپ واقعی "%s" حذف کرنا چاہتے ہیں؟',
    'ru': 'Вы уверены, что хотите удалить «%s»?',
    'ky': '«%s» өчүрүүнү чын эле каалайсызбы?',
    'az': '"%s" silmək istədiyinizə əminsiniz?',
    'ka': 'დარწმუნებული ხართ, რომ გსურთ "%s"-ის წაშლა?',
  },
  'adm_cannot_delete_self': {
    'en': 'You cannot delete your own account',
    'pt': 'You cannot delete your own account',
    'fr': 'You cannot delete your own account',
    'ar': 'لا يمكنك حذف حسابك الحالي',
    'zh_Hans': '您不能删除自己的账户',
    'tr': 'Kendi hesabınızı silemezsiniz',
    'ur': 'آپ اپنا موجودہ اکاؤنٹ حذف نہیں کر سکتے',
    'ru': 'Нельзя удалить свой аккаунт',
    'ky': 'Өз аккаунтуңузду өчүрө албайсыз',
    'az': 'Öz hesabınızı silə bilməzsiniz',
    'ka': 'საკუთარი ანგარიშის წაშლა შეუძლებელია',
  },

  // Validation
  'adm_select_country': {
    'en': 'Please select a country',
    'pt': 'Por favor select a country',
    'fr': 'Veuillez select a country',
    'ar': 'يرجى اختيار الدولة',
    'zh_Hans': '请选择国家',
    'tr': 'Lütfen bir ülke seçin',
    'ur': 'براہ کرم ملک منتخب کریں',
    'ru': 'Выберите страну',
    'ky': 'Өлкөнү тандаңыз',
    'az': 'Zəhmət olmasa ölkə seçin',
    'ka': 'აირჩიეთ ქვეყანა',
  },
  'adm_select_country_first': {
    'en': 'Please select a country first',
    'pt': 'Por favor select a country first',
    'fr': 'Veuillez select a country first',
    'ar': 'يرجى اختيار الدولة أولاً',
    'zh_Hans': '请先选择国家',
    'tr': 'Önce bir ülke seçin',
    'ur': 'پہلے ملک منتخب کریں',
    'ru': 'Сначала выберите страну',
    'ky': 'Адегенде өлкөнү тандаңыз',
    'az': 'Əvvəlcə ölkə seçin',
    'ka': 'ჯერ აირჩიეთ ქვეყანა',
  },
  'adm_select_region': {
    'en': 'Please select a region',
    'pt': 'Por favor select a region',
    'fr': 'Veuillez select a region',
    'ar': 'يرجى اختيار المنطقة',
    'zh_Hans': '请选择地区',
    'tr': 'Lütfen bir bölge seçin',
    'ur': 'براہ کرم علاقہ منتخب کریں',
    'ru': 'Выберите регион',
    'ky': 'Регионду тандаңыз',
    'az': 'Zəhmət olmasa region seçin',
    'ka': 'აირჩიეთ რეგიონი',
  },
  'adm_select_city': {
    'en': 'Please select a city',
    'pt': 'Por favor select a city',
    'fr': 'Veuillez select a city',
    'ar': 'يرجى اختيار المدينة',
    'zh_Hans': '请选择城市',
    'tr': 'Lütfen bir şehir seçin',
    'ur': 'براہ کرم شہر منتخب کریں',
    'ru': 'Выберите город',
    'ky': 'Шаарды тандаңыз',
    'az': 'Zəhmət olmasa şəhər seçin',
    'ka': 'აირჩიეთ ქალაქი',
  },
  'adm_enter_region_name': {
    'en': 'Please enter the region name',
    'pt': 'Por favor enter the region name',
    'fr': 'Veuillez enter the region name',
    'ar': 'يرجى إدخال اسم المنطقة',
    'zh_Hans': '请输入地区名称',
    'tr': 'Bölge adını girin',
    'ur': 'علاقے کا نام درج کریں',
    'ru': 'Введите название региона',
    'ky': 'Региондун аталышын киргизиңиз',
    'az': 'Region adını daxil edin',
    'ka': 'შეიყვანეთ რეგიონის სახელი',
  },
  'adm_enter_city_name': {
    'en': 'Please enter the city name',
    'pt': 'Por favor enter the city name',
    'fr': 'Veuillez enter the city name',
    'ar': 'يرجى إدخال اسم المدينة',
    'zh_Hans': '请输入城市名称',
    'tr': 'Şehir adını girin',
    'ur': 'شہر کا نام درج کریں',
    'ru': 'Введите название города',
    'ky': 'Шаардын аталышын киргизиңиз',
    'az': 'Şəhər adını daxil edin',
    'ka': 'შეიყვანეთ ქალაქის სახელი',
  },
  'adm_wait_image_upload': {
    'en': 'Wait for the image upload to finish, then save',
    'pt': 'Wait for the image upload to finish, then save',
    'fr': 'Wait for the image upload to finish, then save',
    'ar': 'انتظر اكتمال رفع الصورة ثم احفظ',
    'zh_Hans': '请等待图片上传完成后再保存',
    'tr': 'Görsel yüklemesi bitene kadar bekleyin, sonra kaydedin',
    'ur': 'تصویر اپ لوڈ مکمل ہونے کا انتظار کریں پھر محفوظ کریں',
    'ru': 'Дождитесь загрузки изображения, затем сохраните',
    'ky': 'Сүрөт жүктөлгүчө күтүп, андан кийин сактаңыз',
    'az': 'Şəkil yüklənənə qədər gözləyin, sonra saxlayın',
    'ka': 'დაელოდეთ სურათის ატვირთვას, შემდეგ შეინახეთ',
  },
  'adm_wait_images_upload': {
    'en': 'Wait for image uploads to finish, then save',
    'pt': 'Wait for image uploads to finish, then save',
    'fr': 'Wait for image uploads to finish, then save',
    'ar': 'انتظر اكتمال رفع الصور ثم احفظ',
    'zh_Hans': '请等待图片上传完成后再保存',
    'tr': 'Görsel yüklemeleri bitene kadar bekleyin, sonra kaydedin',
    'ur': 'تصاویر اپ لوڈ مکمل ہونے کا انتظار کریں پھر محفوظ کریں',
    'ru': 'Дождитесь загрузки изображений, затем сохраните',
    'ky': 'Сүрөттөр жүктөлгүчө күтүп, андан кийин сактаңыз',
    'az': 'Şəkillər yüklənənə qədər gözləyin, sonra saxlayın',
    'ka': 'დაელოდეთ სურათების ატვირთვას, შემდეგ შეინახეთ',
  },
  'adm_password_mismatch': {
    'en': 'Passwords do not match',
    'pt': 'As senhas não coincidem',
    'fr': 'Les mots de passe ne correspondent pas',
    'ar': 'كلمتا المرور غير متطابقتين',
    'zh_Hans': '两次密码不一致',
    'tr': 'Şifreler eşleşmiyor',
    'ur': 'پاس ورڈز مماثل نہیں',
    'ru': 'Пароли не совпадают',
    'ky': 'Сырсөздөр дал келбейт',
    'az': 'Şifrələr uyğun gəlmir',
    'ka': 'პაროლები არ ემთხვევა',
  },
  'adm_password_min_6': {
    'en': 'Password must be at least 6 characters',
    'pt': 'Password must be at least 6 characters',
    'fr': 'Password must be at least 6 characters',
    'ar': 'كلمة المرور يجب أن تكون 6 أحرف على الأقل',
    'zh_Hans': '密码至少6个字符',
    'tr': 'Şifre en az 6 karakter olmalı',
    'ur': 'پاس ورڈ کم از کم 6 حروف کا ہونا چاہیے',
    'ru': 'Пароль должен быть не менее 6 символов',
    'ky': 'Сырсөз кеминде 6 белгиден турушу керек',
    'az': 'Şifrə ən azı 6 simvol olmalıdır',
    'ka': 'პაროლი მინიმუმ 6 სიმბოლო უნდა იყოს',
  },
  'adm_confirm_password': {
    'en': 'Confirm password',
    'pt': 'Confirmar password',
    'fr': 'Confirmer password',
    'ar': 'تأكيد كلمة المرور',
    'zh_Hans': '确认密码',
    'tr': 'Şifreyi onayla',
    'ur': 'پاس ورڈ کی تصدیق',
    'ru': 'Подтвердите пароль',
    'ky': 'Сырсөздү ырастаңыз',
    'az': 'Şifrəni təsdiqlə',
    'ka': 'პაროლის დადასტურება',
  },
  'adm_fix_red_fields': {
    'en': 'Please fix the fields highlighted in red',
    'pt': 'Por favor fix the fields highlighted in red',
    'fr': 'Veuillez fix the fields highlighted in red',
    'ar': 'يرجى تصحيح الحقول المحددة بالأحمر',
    'zh_Hans': '请修正标红的字段',
    'tr': 'Kırmızıyla işaretli alanları düzeltin',
    'ur': 'سرخ نشان والے خانے درست کریں',
    'ru': 'Исправьте поля, выделенные красным',
    'ky': 'Кызыл менен белгиленген талааларды оңдоңуз',
    'az': 'Qırmızı ilə işarələnmiş sahələri düzəldin',
    'ka': 'გამოასწორეთ წითლად მონიშნული ველები',
  },

  // Labels
  'adm_country': {
    'en': 'Country',
    'pt': 'País',
    'fr': 'Pays',
    'ar': 'الدولة',
    'zh_Hans': '国家',
    'tr': 'Ülke',
    'ur': 'ملک',
    'ru': 'Страна',
    'ky': 'Өлкө',
    'az': 'Ölkə',
    'ka': 'ქვეყანა',
  },
  'adm_region': {
    'en': 'Region / province',
    'pt': 'Região / província',
    'fr': 'Région / province',
    'ar': 'المنطقة / المحافظة',
    'zh_Hans': '地区/省',
    'tr': 'Bölge / il',
    'ur': 'علاقہ / صوبہ',
    'ru': 'Регион / провинция',
    'ky': 'Регион / облус',
    'az': 'Region / vilayət',
    'ka': 'რეგიონი / პროვინცია',
  },
  'adm_city': {
    'en': 'City',
    'pt': 'Cidade',
    'fr': 'Ville',
    'ar': 'المدينة',
    'zh_Hans': '城市',
    'tr': 'Şehir',
    'ur': 'شہر',
    'ru': 'Город',
    'ky': 'Шаар',
    'az': 'Şəhər',
    'ka': 'ქალაქი',
  },
  'adm_email': {
    'en': 'Email',
    'pt': 'E-mail',
    'fr': 'E-mail',
    'ar': 'البريد الإلكتروني',
    'zh_Hans': '电子邮件',
    'tr': 'E-posta',
    'ur': 'ای میل',
    'ru': 'Эл. почта',
    'ky': 'Электрондук почта',
    'az': 'E-poçt',
    'ka': 'ელფოსტა',
  },
  'adm_full_name': {
    'en': 'Full name',
    'pt': 'Nome completo',
    'fr': 'Nom complet',
    'ar': 'الاسم الكامل',
    'zh_Hans': '全名',
    'tr': 'Ad soyad',
    'ur': 'مکمل نام',
    'ru': 'Полное имя',
    'ky': 'Толук аты',
    'az': 'Tam ad',
    'ka': 'სრული სახელი',
  },
  'adm_phone': {
    'en': 'Mobile number',
    'pt': 'Número de celular',
    'fr': 'Numéro de mobile',
    'ar': 'رقم الجوال',
    'zh_Hans': '手机号',
    'tr': 'Cep telefonu',
    'ur': 'موبائل نمبر',
    'ru': 'Мобильный номер',
    'ky': 'Мобилдик номер',
    'az': 'Mobil nömrə',
    'ka': 'მობილური ნომერი',
  },
  'adm_password': {
    'en': 'Password',
    'pt': 'Senha',
    'fr': 'Mot de passe',
    'ar': 'كلمة المرور',
    'zh_Hans': '密码',
    'tr': 'Şifre',
    'ur': 'پاس ورڈ',
    'ru': 'Пароль',
    'ky': 'Сырсөз',
    'az': 'Şifrə',
    'ka': 'პაროლი',
  },
  'adm_basic_data': {
    'en': 'Basic information',
    'pt': 'Informações básicas',
    'fr': 'Informations de base',
    'ar': 'البيانات الأساسية',
    'zh_Hans': '基本信息',
    'tr': 'Temel bilgiler',
    'ur': 'بنیادی معلومات',
    'ru': 'Основные данные',
    'ky': 'Негизги маалымат',
    'az': 'Əsas məlumatlar',
    'ka': 'ძირითადი მონაცემები',
  },
  'adm_location': {
    'en': 'Geographic location',
    'pt': 'Localização geográfica',
    'fr': 'Localisation géographique',
    'ar': 'الموقع الجغرافي',
    'zh_Hans': '地理位置',
    'tr': 'Coğrafi konum',
    'ur': 'جغرافیائی مقام',
    'ru': 'Географическое положение',
    'ky': 'Географиялык жайгашуу',
    'az': 'Coğrafi mövqe',
    'ka': 'გეოგრაფიული მდებარეობა',
  },

  // Pickers
  'adm_pick_country': {
    'en': 'Select country',
    'pt': 'Selecionar país',
    'fr': 'Sélectionner un pays',
    'ar': 'اختر الدولة',
    'zh_Hans': '选择国家',
    'tr': 'Ülke seç',
    'ur': 'ملک منتخب کریں',
    'ru': 'Выберите страну',
    'ky': 'Өлкөнү тандаңыз',
    'az': 'Ölkə seçin',
    'ka': 'აირჩიეთ ქვეყანა',
  },
  'adm_pick_region': {
    'en': 'Select region',
    'pt': 'Selecionar região',
    'fr': 'Sélectionner une région',
    'ar': 'اختر المنطقة',
    'zh_Hans': '选择地区',
    'tr': 'Bölge seç',
    'ur': 'علاقہ منتخب کریں',
    'ru': 'Выберите регион',
    'ky': 'Регионду тандаңыз',
    'az': 'Region seçin',
    'ka': 'აირჩიეთ რეგიონი',
  },
  'adm_pick_city': {
    'en': 'Select city',
    'pt': 'Selecionar cidade',
    'fr': 'Sélectionner une ville',
    'ar': 'اختر المدينة',
    'zh_Hans': '选择城市',
    'tr': 'Şehir seç',
    'ur': 'شہر منتخب کریں',
    'ru': 'Выберите город',
    'ky': 'Шаарды тандаңыз',
    'az': 'Şəhər seçin',
    'ka': 'აირჩიეთ ქალაქი',
  },
  'adm_pick_work_city': {
    'en': 'Select work city',
    'pt': 'Selecionar cidade de trabalho',
    'fr': 'Sélectionner la ville de travail',
    'ar': 'اختر مدينة العمل',
    'zh_Hans': '选择工作城市',
    'tr': 'Çalışma şehri seç',
    'ur': 'کام کا شہر منتخب کریں',
    'ru': 'Выберите город работы',
    'ky': 'Иштөө шаарын тандаңыз',
    'az': 'İş şəhəri seçin',
    'ka': 'აირჩიეთ სამუშაო ქალაქი',
  },
  'adm_pick_car_type': {
    'en': 'Select car type',
    'pt': 'Selecionar tipo de carro',
    'fr': 'Sélectionner le type de voiture',
    'ar': 'اختر نوع السيارة',
    'zh_Hans': '选择车型',
    'tr': 'Araç tipi seç',
    'ur': 'گاڑی کی قسم منتخب کریں',
    'ru': 'Выберите тип автомобиля',
    'ky': 'Унаанын түрүн тандаңыз',
    'az': 'Avtomobil növü seçin',
    'ka': 'აირჩიეთ მანქანის ტიპი',
  },
  'adm_search_country': {
    'en': 'Search country...',
    'pt': 'Pesquisar country...',
    'fr': 'Rechercher country...',
    'ar': 'بحث عن دولة...',
    'zh_Hans': '搜索国家...',
    'tr': 'Ülke ara...',
    'ur': 'ملک تلاش کریں...',
    'ru': 'Поиск страны...',
    'ky': 'Өлкөнү издөө...',
    'az': 'Ölkə axtar...',
    'ka': 'ქვეყნის ძებნა...',
  },
  'adm_search_region': {
    'en': 'Search region...',
    'pt': 'Pesquisar region...',
    'fr': 'Rechercher region...',
    'ar': 'بحث عن منطقة...',
    'zh_Hans': '搜索地区...',
    'tr': 'Bölge ara...',
    'ur': 'علاقہ تلاش کریں...',
    'ru': 'Поиск региона...',
    'ky': 'Регионду издөө...',
    'az': 'Region axtar...',
    'ka': 'რეგიონის ძებნა...',
  },
  'adm_search_city': {
    'en': 'Search city...',
    'pt': 'Pesquisar city...',
    'fr': 'Rechercher city...',
    'ar': 'بحث عن مدينة...',
    'zh_Hans': '搜索城市...',
    'tr': 'Şehir ara...',
    'ur': 'شہر تلاش کریں...',
    'ru': 'Поиск города...',
    'ky': 'Шаарды издөө...',
    'az': 'Şəhər axtar...',
    'ka': 'ქალაქის ძებნა...',
  },
  'adm_no_countries': {
    'en': 'No countries registered',
    'pt': 'Não countries registered',
    'fr': 'Non countries registered',
    'ar': 'لا توجد دول مسجلة',
    'zh_Hans': '没有注册的国家',
    'tr': 'Kayıtlı ülke yok',
    'ur': 'کوئی ملک رجسٹرڈ نہیں',
    'ru': 'Нет зарегистрированных стран',
    'ky': 'Катталган өлкөлөр жок',
    'az': 'Qeydiyyatlı ölkə yoxdur',
    'ka': 'რეგისტრირებული ქვეყნები არ არის',
  },
  'adm_no_regions_country': {
    'en': 'No regions for this country',
    'pt': 'Não regions for this country',
    'fr': 'Non regions for this country',
    'ar': 'لا توجد مناطق لهذه الدولة',
    'zh_Hans': '该国家没有地区',
    'tr': 'Bu ülke için bölge yok',
    'ur': 'اس ملک کے لیے کوئی علاقہ نہیں',
    'ru': 'Нет регионов для этой страны',
    'ky': 'Бул өлкө үчүн региондор жок',
    'az': 'Bu ölkə üçün region yoxdur',
    'ka': 'ამ ქვეყნის რეგიონები არ არის',
  },
  'adm_no_regions_pick_country': {
    'en': 'No regions — select a country first',
    'pt': 'Não regions — select a country first',
    'fr': 'Non regions — select a country first',
    'ar': 'لا توجد مناطق — اختر الدولة أولاً',
    'zh_Hans': '没有地区 — 请先选择国家',
    'tr': 'Bölge yok — önce ülke seçin',
    'ur': 'کوئی علاقہ نہیں — پہلے ملک منتخب کریں',
    'ru': 'Нет регионов — сначала выберите страну',
    'ky': 'Региондор жок — адегенде өлкөнү тандаңыз',
    'az': 'Region yoxdur — əvvəlcə ölkə seçin',
    'ka': 'რეგიონები არ არის — ჯერ აირჩიეთ ქვეყანა',
  },
  'adm_pick_country_filter_regions': {
    'en': 'Select a country first to filter regions',
    'pt': 'Select a country first to filter regions',
    'fr': 'Select a country first to filter regions',
    'ar': 'اختر الدولة أولاً لتصفية المناطق',
    'zh_Hans': '请先选择国家以筛选地区',
    'tr': 'Bölgeleri filtrelemek için önce ülke seçin',
    'ur': 'علاقے فلٹر کرنے کے لیے پہلے ملک منتخب کریں',
    'ru': 'Сначала выберите страну для фильтрации регионов',
    'ky': 'Региондорду чыпкалоо үчүн адегенде өлкөнү тандаңыз',
    'az': 'Regionları süzmək üçün əvvəlcə ölkə seçin',
    'ka': 'რეგიონების ფილტრაციისთვის ჯერ აირჩიეთ ქვეყანა',
  },
  'adm_no_cities': {
    'en': 'No cities available',
    'pt': 'Não cities available',
    'fr': 'Non cities available',
    'ar': 'لا توجد مدن متاحة',
    'zh_Hans': '没有可用城市',
    'tr': 'Kullanılabilir şehir yok',
    'ur': 'کوئی شہر دستیاب نہیں',
    'ru': 'Нет доступных городов',
    'ky': 'Шаарлар жок',
    'az': 'Mövcud şəhər yoxdur',
    'ka': 'ხელმისაწვდომი ქალაქები არ არის',
  },
  'adm_agent_cities_only': {
    'en': 'Showing cities in the agent country only',
    'pt': 'Showing cities in the agent country only',
    'fr': 'Showing cities in the agent country only',
    'ar': 'عرض مدن الدولة المحددة للوكيل فقط',
    'zh_Hans': '仅显示代理所在国家的城市',
    'tr': 'Yalnızca acentenin ülkesindeki şehirler gösteriliyor',
    'ur': 'صرف ایجنٹ کے ملک کے شہر دکھائے جا رہے ہیں',
    'ru': 'Показаны только города страны агента',
    'ky': 'Агенттин өлкөсүндөгү шаарлар гана көрсөтүлүүдө',
    'az': 'Yalnız agentin ölkəsinin şəhərləri göstərilir',
    'ka': 'ჩანს მხოლოდ აგენტის ქვეყნის ქალაქები',
  },
  'adm_region_filter_cities': {
    'en': 'You can select a region first to filter cities',
    'pt': 'You can select a region first to filter cities',
    'fr': 'You can select a region first to filter cities',
    'ar': 'يمكنك اختيار منطقة أولاً لتصفية المدن',
    'zh_Hans': '可先选择地区以筛选城市',
    'tr': 'Şehirleri filtrelemek için önce bölge seçebilirsiniz',
    'ur': 'شہروں کو فلٹر کرنے کے لیے پہلے علاقہ منتخب کر سکتے ہیں',
    'ru': 'Можно сначала выбрать регион для фильтрации городов',
    'ky': 'Шаарларды чыпкалоо үчүн адегенде регионду тандай аласыз',
    'az': 'Şəhərləri süzmək üçün əvvəlcə region seçə bilərsiniz',
    'ka': 'ქალაქების ფილტრაციისთვის ჯერ აირჩიეთ რეგიონი',
  },

  // List / network
  'adm_server_refresh_failed': {
    'en': 'Could not refresh data from server',
    'pt': 'Could not refresh data from server',
    'fr': 'Could not refresh data from server',
    'ar': 'تعذر تحديث البيانات من الخادم',
    'zh_Hans': '无法从服务器刷新数据',
    'tr': 'Sunucudan veri yenilenemedi',
    'ur': 'سرور سے ڈیٹا تازہ نہیں ہو سکا',
    'ru': 'Не удалось обновить данные с сервера',
    'ky': 'Серверден маалымат жаңыланган жок',
    'az': 'Serverdən məlumat yenilənmədi',
    'ka': 'სერვერიდან მონაცემების განახლება ვერ მოხერხდა',
  },
  'adm_scope_not_ready': {
    'en': 'Country scope is still loading. Retry in a moment.',
    'pt': 'Country scope is still loading. Retry in a moment.',
    'fr': 'Country scope is still loading. Retry in a moment.',
    'ar': 'نطاق الدولة ما زال يُحمَّل. أعد المحاولة بعد لحظات.',
    'zh_Hans': '国家范围仍在加载。请稍后重试。',
    'tr': 'Ülke kapsamı hâlâ yükleniyor. Biraz sonra yeniden deneyin.',
    'ur': 'ملک کا دائرہ ابھی لوڈ ہو رہا ہے۔ تھوڑی دیر بعد دوبارہ کوشش کریں۔',
    'ru': 'Область страны ещё загружается. Повторите через мгновение.',
    'ky': 'Өлкө чөйрөсү дагы эле жүктөлүүдө. Бир аздан кийин кайталаңыз.',
    'az': 'Ölkə əhatəsi hələ yüklənir. Bir az sonra yenidən cəhd edin.',
    'ka': 'ქვეყნის დიაპაზონი ჯერ კიდევ იტვირთება. ცოტა ხანში სცადეთ თავიდან.',
  },
  'adm_load_failed_network': {
    'en': 'Could not load data. Check your connection and try again.',
    'pt': 'Could not load data. Check your connection and try again.',
    'fr': 'Could not load data. Check your connection and try again.',
    'ar': 'تعذر تحميل البيانات. تحقق من الاتصال وحاول مرة أخرى.',
    'zh_Hans': '无法加载数据。请检查网络后重试。',
    'tr': 'Veri yüklenemedi. Bağlantınızı kontrol edip tekrar deneyin.',
    'ur': 'ڈیٹا لوڈ نہیں ہو سکا۔ کنکشن چیک کریں اور دوبارہ کوشش کریں۔',
    'ru': 'Не удалось загрузить данные. Проверьте подключение и повторите.',
    'ky': 'Маалымат жүктөлгөн жок. Байланышты текшерип, кайра аракет кылыңыз.',
    'az': 'Məlumat yüklənmədi. Bağlantını yoxlayın və yenidən cəhd edin.',
    'ka':
        'მონაცემების ჩატვირთვა ვერ მოხერხდა. შეამოწმეთ კავშირი და სცადეთ თავიდან.',
  },
  'adm_load_more_failed': {
    'en': 'Could not load more records',
    'pt': 'Não foi possível carregar mais registros',
    'fr': 'Impossible de charger plus d\'enregistrements',
    'ar': 'تعذر تحميل المزيد من السجلات',
    'zh_Hans': '无法加载更多记录',
    'tr': 'Daha fazla kayıt yüklenemedi',
    'ur': 'مزید ریکارڈز لوڈ نہیں ہو سکے',
    'ru': 'Не удалось загрузить больше записей',
    'ky': 'Дагы жазуулар жүктөлгөн жок',
    'az': 'Daha çox qeyd yüklənmədi',
    'ka': 'მეტი ჩანაწერის ჩატვირთვა ვერ მოხერხდა',
  },
  'adm_load_failed': {
    'en': 'Could not load data',
    'pt': 'Não foi possível carregar os dados',
    'fr': 'Impossible de charger les données',
    'ar': 'تعذر تحميل البيانات',
    'zh_Hans': '无法加载数据',
    'tr': 'Veri yüklenemedi',
    'ur': 'ڈیٹا لوڈ نہیں ہو سکا',
    'ru': 'Не удалось загрузить данные',
    'ky': 'Маалымат жүктөлгөн жок',
    'az': 'Məlumat yüklənmədi',
    'ka': 'მონაცემების ჩატვირთვა ვერ მოხერხდა',
  },
  'adm_records_shown': {
    'en': 'Showing %s records',
    'pt': 'Mostrando %s registros',
    'fr': 'Affichage de %s enregistrements',
    'ar': 'تم عرض %s سجل',
    'zh_Hans': '已显示 %s 条记录',
    'tr': '%s kayıt gösteriliyor',
    'ur': '%s ریکارڈز دکھائے گئے',
    'ru': 'Показано записей: %s',
    'ky': '%s жазуу көрсөтүлүүдө',
    'az': '%s qeyd göstərilir',
    'ka': 'ნაჩვენებია %s ჩანაწერი',
  },

  // Add screens
  'adm_add_region_title': {
    'en': 'Add new region',
    'pt': 'Adicionar new region',
    'fr': 'Ajouter new region',
    'ar': 'إضافة منطقة جديدة',
    'zh_Hans': '添加新地区',
    'tr': 'Yeni bölge ekle',
    'ur': 'نیا علاقہ شامل کریں',
    'ru': 'Добавить регион',
    'ky': 'Жаңы регион кошуу',
    'az': 'Yeni region əlavə et',
    'ka': 'ახალი რეგიონის დამატება',
  },
  'adm_add_region_subtitle_agent': {
    'en': 'Agent country is fixed — enter region details',
    'pt': 'Agent country is fixed — enter region details',
    'fr': 'Agent country is fixed — enter region details',
    'ar': 'دولة الوكيل ثابتة — أدخل بيانات المنطقة / المحافظة',
    'zh_Hans': '代理国家已固定 — 请输入地区信息',
    'tr': 'Acente ülkesi sabit — bölge bilgilerini girin',
    'ur': 'ایجنٹ کا ملک مقفل ہے — علاقے کی تفصیلات درج کریں',
    'ru': 'Страна агента фиксирована — введите данные региона',
    'ky': 'Агенттин өлкөсү бекитилген — регион маалыматтарын киргизиңиз',
    'az': 'Agent ölkəsi sabitdir — region məlumatlarını daxil edin',
    'ka': 'აგენტის ქვეყანა ფიქსირებულია — შეიყვანეთ რეგიონის მონაცემები',
  },
  'adm_add_region_subtitle': {
    'en': 'Select a country, then enter region details',
    'pt': 'Select a country, then enter region details',
    'fr': 'Select a country, then enter region details',
    'ar': 'اختر الدولة ثم أدخل بيانات المنطقة / المحافظة',
    'zh_Hans': '选择国家后输入地区信息',
    'tr': 'Ülke seçin, ardından bölge bilgilerini girin',
    'ur': 'ملک منتخب کریں پھر علاقے کی تفصیلات درج کریں',
    'ru': 'Выберите страну и введите данные региона',
    'ky': 'Өлкөнү тандап, регион маалыматтарын киргизиңиз',
    'az': 'Ölkə seçin, sonra region məlumatlarını daxil edin',
    'ka': 'აირჩიეთ ქვეყანა, შემდეგ შეიყვანეთ რეგიონის მონაცემები',
  },
  'adm_save_region': {
    'en': 'Save region',
    'pt': 'Salvar region',
    'fr': 'Enregistrer region',
    'ar': 'حفظ المنطقة',
    'zh_Hans': '保存地区',
    'tr': 'Bölgeyi kaydet',
    'ur': 'علاقہ محفوظ کریں',
    'ru': 'Сохранить регион',
    'ky': 'Регионду сактоо',
    'az': 'Regionu saxla',
    'ka': 'რეგიონის შენახვა',
  },
  'adm_region_added': {
    'en': 'Region added successfully',
    'pt': 'Região adicionada com sucesso',
    'fr': 'Région ajoutée avec succès',
    'ar': 'تم إضافة المنطقة بنجاح',
    'zh_Hans': '地区添加成功',
    'tr': 'Bölge başarıyla eklendi',
    'ur': 'علاقہ کامیابی سے شامل ہو گیا',
    'ru': 'Регион успешно добавлен',
    'ky': 'Регион ийгиликтүү кошулду',
    'az': 'Region uğurla əlavə edildi',
    'ka': 'რეგიონი წარმატებით დაემატა',
  },
  'adm_add_city_title': {
    'en': 'Add new city',
    'pt': 'Adicionar new city',
    'fr': 'Ajouter new city',
    'ar': 'إضافة مدينة جديدة',
    'zh_Hans': '添加新城市',
    'tr': 'Yeni şehir ekle',
    'ur': 'نیا شہر شامل کریں',
    'ru': 'Добавить город',
    'ky': 'Жаңы шаар кошуу',
    'az': 'Yeni şəhər əlavə et',
    'ka': 'ახალი ქალაქის დამატება',
  },
  'adm_add_city_subtitle_agent': {
    'en': 'Agent country is fixed — select a region, then enter city details',
    'pt': 'Agent country is fixed — select a region, then enter city details',
    'fr': 'Agent country is fixed — select a region, then enter city details',
    'ar': 'دولة الوكيل ثابتة — اختر المنطقة ثم أدخل بيانات المدينة',
    'zh_Hans': '代理国家已固定 — 选择地区后输入城市信息',
    'tr': 'Acente ülkesi sabit — bölge seçin, ardından şehir bilgilerini girin',
    'ur': 'ایجنٹ کا ملک مقفل ہے — علاقہ منتخب کریں پھر شہر کی تفصیلات درج کریں',
    'ru': 'Страна агента фиксирована — выберите регион и введите данные города',
    'ky':
        'Агенттин өлкөсү бекитилген — регионду тандап, шаар маалыматтарын киргизиңиз',
    'az':
        'Agent ölkəsi sabitdir — region seçin, sonra şəhər məlumatlarını daxil edin',
    'ka':
        'აგენტის ქვეყანა ფიქსირებულია — აირჩიეთ რეგიონი, შემდეგ შეიყვანეთ ქალაქის მონაცემები',
  },
  'adm_add_city_subtitle': {
    'en': 'Select country and region, then enter city details',
    'pt': 'Select country and region, then enter city details',
    'fr': 'Select country and region, then enter city details',
    'ar': 'اختر الدولة والمنطقة ثم أدخل بيانات المدينة',
    'zh_Hans': '选择国家和地区后输入城市信息',
    'tr': 'Ülke ve bölge seçin, ardından şehir bilgilerini girin',
    'ur': 'ملک اور علاقہ منتخب کریں پھر شہر کی تفصیلات درج کریں',
    'ru': 'Выберите страну и регион, затем введите данные города',
    'ky': 'Өлкө менен регионду тандап, шаар маалыматтарын киргизиңиз',
    'az': 'Ölkə və region seçin, sonra şəhər məlumatlarını daxil edin',
    'ka': 'აირჩიეთ ქვეყანა და რეგიონი, შემდეგ შეიყვანეთ ქალაქის მონაცემები',
  },
  'adm_save_city': {
    'en': 'Save city',
    'pt': 'Salvar city',
    'fr': 'Enregistrer city',
    'ar': 'حفظ المدينة',
    'zh_Hans': '保存城市',
    'tr': 'Şehri kaydet',
    'ur': 'شہر محفوظ کریں',
    'ru': 'Сохранить город',
    'ky': 'Шаарды сактоо',
    'az': 'Şəhəri saxla',
    'ka': 'ქალაქის შენახვა',
  },
  'adm_city_added': {
    'en': 'City added successfully',
    'pt': 'Cidade added successfully',
    'fr': 'Ville added successfully',
    'ar': 'تم إضافة المدينة بنجاح',
    'zh_Hans': '城市添加成功',
    'tr': 'Şehir başarıyla eklendi',
    'ur': 'شہر کامیابی سے شامل ہو گیا',
    'ru': 'Город успешно добавлен',
    'ky': 'Шаар ийгиликтүү кошулду',
    'az': 'Şəhər uğurla əlavə edildi',
    'ka': 'ქალაქი წარმატებით დაემატა',
  },
  'adm_region_image': {
    'en': 'Region image',
    'pt': 'Imagem da região',
    'fr': 'Image de la région',
    'ar': 'صورة المنطقة',
    'zh_Hans': '地区图片',
    'tr': 'Bölge görseli',
    'ur': 'علاقے کی تصویر',
    'ru': 'Изображение региона',
    'ky': 'Региондун сүрөтү',
    'az': 'Region şəkli',
    'ka': 'რეგიონის სურათი',
  },
  'adm_city_image': {
    'en': 'City image',
    'pt': 'Cidade image',
    'fr': 'Ville image',
    'ar': 'صورة المدينة',
    'zh_Hans': '城市图片',
    'tr': 'Şehir görseli',
    'ur': 'شہر کی تصویر',
    'ru': 'Изображение города',
    'ky': 'Шаардын сүрөтү',
    'az': 'Şəhər şəkli',
    'ka': 'ქალაქის სურათი',
  },
  'adm_pick_region_image': {
    'en': 'Tap to choose a region image',
    'pt': 'Tap to choose a region image',
    'fr': 'Tap to choose a region image',
    'ar': 'اضغط لاختيار صورة المنطقة',
    'zh_Hans': '点击选择地区图片',
    'tr': 'Bölge görseli seçmek için dokunun',
    'ur': 'علاقے کی تصویر منتخب کرنے کے لیے ٹیپ کریں',
    'ru': 'Нажмите, чтобы выбрать изображение региона',
    'ky': 'Региондун сүрөтүн тандоо үчүн басыңыз',
    'az': 'Region şəkli seçmək üçün toxunun',
    'ka': 'რეგიონის სურათის ასარჩევად შეეხეთ',
  },
  'adm_pick_city_image': {
    'en': 'Tap to choose a city image',
    'pt': 'Toque para escolher uma imagem da cidade',
    'fr': 'Appuyez pour choisir une image de ville',
    'ar': 'اضغط لاختيار صورة المدينة',
    'zh_Hans': '点击选择城市图片',
    'tr': 'Şehir görseli seçmek için dokunun',
    'ur': 'شہر کی تصویر منتخب کرنے کے لیے ٹیپ کریں',
    'ru': 'Нажмите, чтобы выбрать изображение города',
    'ky': 'Шаардын сүрөтүн тандоо үчүн басыңыз',
    'az': 'Şəhər şəkli seçmək üçün toxunun',
    'ka': 'ქალაქის სურათის ასარჩევად შეეხეთ',
  },
  'adm_region_name_label': {
    'en': 'Region / province name',
    'pt': 'Nome da região / província',
    'fr': 'Nom de la région / province',
    'ar': 'اسم المنطقة / المحافظة',
    'zh_Hans': '地区/省名称',
    'tr': 'Bölge / il adı',
    'ur': 'علاقہ / صوبے کا نام',
    'ru': 'Название региона / провинции',
    'ky': 'Регион / облус аталышы',
    'az': 'Region / vilayət adı',
    'ka': 'რეგიონის / პროვინციის სახელი',
  },
  'adm_region_name_hint': {
    'en': 'e.g. Riyadh Region',
    'pt': 'ex.: Região de Riade',
    'fr': 'ex. Région de Riyad',
    'ar': 'مثال: منطقة الرياض',
    'zh_Hans': '例如：利雅得地区',
    'tr': 'örn. Riyad Bölgesi',
    'ur': 'مثال: ریاض علاقہ',
    'ru': 'напр. регион Эр-Рияд',
    'ky': 'мис. Эр-Рияд региону',
    'az': 'məs. Riyad regionu',
    'ka': 'მაგ. ერ-რიადის რეგიონი',
  },
  'adm_city_name_label': {
    'en': 'City name',
    'pt': 'Cidade name',
    'fr': 'Ville name',
    'ar': 'اسم المدينة',
    'zh_Hans': '城市名称',
    'tr': 'Şehir adı',
    'ur': 'شہر کا نام',
    'ru': 'Название города',
    'ky': 'Шаардын аталышы',
    'az': 'Şəhər adı',
    'ka': 'ქალაქის სახელი',
  },
  'adm_city_name_hint': {
    'en': 'e.g. Riyadh',
    'pt': 'ex.: Riade',
    'fr': 'ex. Riyad',
    'ar': 'مثال: الرياض',
    'zh_Hans': '例如：利雅得',
    'tr': 'örn. Riyad',
    'ur': 'مثال: ریاض',
    'ru': 'напр. Эр-Рияд',
    'ky': 'мис. Эр-Рияд',
    'az': 'məs. Riyad',
    'ka': 'მაგ. ერ-რიადი',
  },
  'adm_region_desc_label': {
    'en': 'Region description (optional)',
    'pt': 'Region description (optional)',
    'fr': 'Region description (optional)',
    'ar': 'وصف المنطقة (اختياري)',
    'zh_Hans': '地区描述（可选）',
    'tr': 'Bölge açıklaması (isteğe bağlı)',
    'ur': 'علاقے کی تفصیل (اختیاری)',
    'ru': 'Описание региона (необязательно)',
    'ky': 'Региондун сүрөттөмөсү (милдеттүү эмес)',
    'az': 'Region təsviri (istəyə bağlı)',
    'ka': 'რეგიონის აღწერა (არასავალდებულო)',
  },
  'adm_city_desc_label': {
    'en': 'City description (optional)',
    'pt': 'Cidade description (optional)',
    'fr': 'Ville description (optional)',
    'ar': 'وصف المدينة (اختياري)',
    'zh_Hans': '城市描述（可选）',
    'tr': 'Şehir açıklaması (isteğe bağlı)',
    'ur': 'شہر کی تفصیل (اختیاری)',
    'ru': 'Описание города (необязательно)',
    'ky': 'Шаардын сүрөттөмөсү (милдеттүү эмес)',
    'az': 'Şəhər təsviri (istəyə bağlı)',
    'ka': 'ქალაქის აღწერა (არასავალდებულო)',
  },
  'adm_activate_region': {
    'en': 'Activate region',
    'pt': 'Ativar região',
    'fr': 'Activer la région',
    'ar': 'تفعيل المنطقة',
    'zh_Hans': '启用地区',
    'tr': 'Bölgeyi etkinleştir',
    'ur': 'علاقہ فعال کریں',
    'ru': 'Активировать регион',
    'ky': 'Регионду иштетүү',
    'az': 'Regionu aktivləşdir',
    'ka': 'რეგიონის გააქტიურება',
  },
  'adm_activate_city': {
    'en': 'Activate city',
    'pt': 'Ativar cidade',
    'fr': 'Activer la ville',
    'ar': 'تفعيل المدينة',
    'zh_Hans': '启用城市',
    'tr': 'Şehri etkinleştir',
    'ur': 'شہر فعال کریں',
    'ru': 'Активировать город',
    'ky': 'Шаарды иштетүү',
    'az': 'Şəhəri aktivləşdir',
    'ka': 'ქალაქის გააქტიურება',
  },
  'adm_region_visible_hint': {
    'en': 'Region appears in the app when active',
    'pt': 'Region appears in the app when active',
    'fr': 'Region appears in the app when active',
    'ar': 'تظهر المنطقة في التطبيق عند التفعيل',
    'zh_Hans': '启用后地区将在应用中显示',
    'tr': 'Etkinleştirildiğinde bölge uygulamada görünür',
    'ur': 'فعال ہونے پر علاقہ ایپ میں نظر آئے گا',
    'ru': 'Регион отображается в приложении при активации',
    'ky': 'Регион активдүү болгондо колдонмодо көрүнөт',
    'az': 'Aktiv olduqda region tətbiqdə görünür',
    'ka': 'აქტივაციისას რეგიონი აპში ჩანს',
  },
  'adm_city_visible_hint': {
    'en': 'City appears in the app when active',
    'pt': 'Cidade appears in the app when active',
    'fr': 'Ville appears in the app when active',
    'ar': 'تظهر المدينة في التطبيق عند التفعيل',
    'zh_Hans': '启用后城市将在应用中显示',
    'tr': 'Etkinleştirildiğinde şehir uygulamada görünür',
    'ur': 'فعال ہونے پر شہر ایپ میں نظر آئے گا',
    'ru': 'Город отображается в приложении при активации',
    'ky': 'Шаар активдүү болгондо колдонмодо көрүнөт',
    'az': 'Aktiv olduqda şəhər tətbiqdə görünür',
    'ka': 'აქტივაციისას ქალაქი აპში ჩანს',
  },
  'adm_no_cities_agent_country': {
    'en': 'No cities in the agent country',
    'pt': 'Não cities in the agent country',
    'fr': 'Non cities in the agent country',
    'ar': 'لا توجد مدن في دولة الوكيل',
    'zh_Hans': '代理所在国家没有城市',
    'tr': 'Acente ülkesinde şehir yok',
    'ur': 'ایجنٹ کے ملک میں کوئی شہر نہیں',
    'ru': 'Нет городов в стране агента',
    'ky': 'Агенттин өлкөсүндө шаарлар жок',
    'az': 'Agent ölkəsində şəhər yoxdur',
    'ka': 'აგენტის ქვეყანაში ქალაქები არ არის',
  },
  'adm_no_cities_in_region': {
    'en': 'No cities in this region',
    'pt': 'Não cities in this region',
    'fr': 'Non cities in this region',
    'ar': 'لا توجد مدن في هذه المنطقة',
    'zh_Hans': '该地区没有城市',
    'tr': 'Bu bölgede şehir yok',
    'ur': 'اس علاقے میں کوئی شہر نہیں',
    'ru': 'Нет городов в этом регионе',
    'ky': 'Бул региондо шаарлар жок',
    'az': 'Bu regionda şəhər yoxdur',
    'ka': 'ამ რეგიონში ქალაქები არ არის',
  },
  'adm_search_work_city': {
    'en': 'Search work city...',
    'pt': 'Pesquisar work city...',
    'fr': 'Rechercher work city...',
    'ar': 'بحث عن مدينة عمل...',
    'zh_Hans': '搜索工作城市...',
    'tr': 'Çalışma şehri ara...',
    'ur': 'کام کا شہر تلاش کریں...',
    'ru': 'Поиск города работы...',
    'ky': 'Иштөө шаарын издөө...',
    'az': 'İş şəhəri axtar...',
    'ka': 'სამუშაო ქალაქის ძებნა...',
  },
  'adm_search_car_type': {
    'en': 'Search car type...',
    'pt': 'Pesquisar car type...',
    'fr': 'Rechercher car type...',
    'ar': 'بحث عن نوع سيارة...',
    'zh_Hans': '搜索车型...',
    'tr': 'Araç tipi ara...',
    'ur': 'گاڑی کی قسم تلاش کریں...',
    'ru': 'Поиск типа автомобиля...',
    'ky': 'Унаанын түрүн издөө...',
    'az': 'Avtomobil növü axtar...',
    'ka': 'მანქანის ტიპის ძებნა...',
  },
  'adm_no_car_types': {
    'en': 'No car types available',
    'pt': 'Não car types available',
    'fr': 'Non car types available',
    'ar': 'لا توجد أنواع سيارات متاحة',
    'zh_Hans': '没有可用车型',
    'tr': 'Kullanılabilir araç tipi yok',
    'ur': 'کوئی گاڑی کی قسم دستیاب نہیں',
    'ru': 'Нет доступных типов автомобилей',
    'ky': 'Унаа түрлөрү жок',
    'az': 'Mövcud avtomobil növü yoxdur',
    'ka': 'ხელმისაწვდომი მანქანის ტიპები არ არის',
  },
  'adm_price_per_hour': {
    'en': '%s SAR / hour',
    'pt': '%s SAR / hour',
    'fr': '%s SAR / hour',
    'ar': '%s ريال / ساعة',
    'zh_Hans': '%s 沙特里亚尔/小时',
    'tr': '%s SAR / saat',
    'ur': '%s ریال / گھنٹہ',
    'ru': '%s SAR / час',
    'ky': '%s SAR / саат',
    'az': '%s SAR / saat',
    'ka': '%s SAR / საათი',
  },
  'adm_short_desc': {
    'en': 'Short description',
    'pt': 'Descrição curta',
    'fr': 'Description courte',
    'ar': 'وصف مختصر',
    'zh_Hans': '简短描述',
    'tr': 'Kısa açıklama',
    'ur': 'مختصر تفصیل',
    'ru': 'Краткое описание',
    'ky': 'Кыскача сүрөттөмө',
    'az': 'Qısa təsvir',
    'ka': 'მოკლე აღწერა',
  },
  'adm_city_desc_hint': {
    'en': 'Brief description of the city',
    'pt': 'Brief description of the city',
    'fr': 'Brief description of the city',
    'ar': 'وصف مختصر عن المدينة',
    'zh_Hans': '城市简要描述',
    'tr': 'Şehir hakkında kısa açıklama',
    'ur': 'شہر کی مختصر تفصیل',
    'ru': 'Краткое описание города',
    'ky': 'Шаардын кыскача сүрөттөмөсү',
    'az': 'Şəhər haqqında qısa təsvir',
    'ka': 'ქალაქის მოკლე აღწერა',
  },
  'adm_data_refresh_failed': {
    'en': 'Could not refresh data',
    'pt': 'Não foi possível atualizar os dados',
    'fr': 'Impossible d\'actualiser les données',
    'ar': 'تعذر تحديث البيانات',
    'zh_Hans': '无法刷新数据',
    'tr': 'Veri yenilenemedi',
    'ur': 'ڈیٹا تازہ نہیں ہو سکا',
    'ru': 'Не удалось обновить данные',
    'ky': 'Маалымат жаңыланган жок',
    'az': 'Məlumat yenilənmədi',
    'ka': 'მონაცემების განახლება ვერ მოხერხდა',
  },
  'adm_stop_confirm_body': {
    'en': 'Are you sure you want to deactivate "%s"?',
    'pt': 'Are you sure you want to deactivate "%s"?',
    'fr': 'Are you sure you want to deactivate "%s"?',
    'ar': 'هل أنت متأكد من إيقاف "%s"؟',
    'zh_Hans': '确定要停用“%s”吗？',
    'tr': '"%s" devre dışı bırakılsın mı?',
    'ur': 'کیا آپ واقعی "%s" کو غیر فعال کرنا چاہتے ہیں؟',
    'ru': 'Вы уверены, что хотите деактивировать «%s»?',
    'ky': '«%s» өчүрүүнү чын эле каалайсызбы?',
    'az': '"%s" deaktiv edilsin?',
    'ka': 'დარწმუნებული ხართ, რომ გსურთ "%s"-ის გამორთვა?',
  },
  'adm_delete_agent_body': {
    'en': 'Are you sure you want to delete agent "%s"?',
    'pt': 'Are you sure you want to delete agent "%s"?',
    'fr': 'Are you sure you want to delete agent "%s"?',
    'ar': 'هل أنت متأكد من حذف الوكيل "%s"؟',
    'zh_Hans': '确定要删除代理“%s”吗？',
    'tr': '"%s" acentesini silmek istediğinizden emin misiniz?',
    'ur': 'کیا آپ واقعی ایجنٹ "%s" حذف کرنا چاہتے ہیں؟',
    'ru': 'Вы уверены, что хотите удалить агента «%s»?',
    'ky': '«%s» агентин өчүрүүнү чын эле каалайсызбы?',
    'az': '"%s" agentini silmək istədiyinizə əminsiniz?',
    'ka': 'დარწმუნებული ხართ, რომ გსურთ აგენტის "%s" წაშლა?',
  },
  'adm_phone_label': {
    'en': 'Phone: %s',
    'pt': 'Telefone: %s',
    'fr': 'Téléphone: %s',
    'ar': 'جوال: %s',
    'zh_Hans': '手机：%s',
    'tr': 'Telefon: %s',
    'ur': 'موبائل: %s',
    'ru': 'Телефон: %s',
    'ky': 'Байланыш: %s',
    'az': 'Telefon: %s',
    'ka': 'ტელეფონი: %s',
  },
  'adm_image_format_unsupported': {
    'en': 'Unsupported image format. Use JPG or PNG',
    'pt': 'Unsupported image format. Use JPG or PNG',
    'fr': 'Unsupported image format. Use JPG or PNG',
    'ar': 'صيغة الصورة غير مدعومة. استخدم JPG أو PNG',
    'zh_Hans': '不支持的图片格式，请使用 JPG 或 PNG',
    'tr': 'Desteklenmeyen görsel formatı. JPG veya PNG kullanın',
    'ur': 'تصویر کی شکل معاون نہیں۔ JPG یا PNG استعمال کریں',
    'ru': 'Неподдерживаемый формат изображения. Используйте JPG или PNG',
    'ky': 'Сүрөт форматы колдоого алынбайт. JPG же PNG колдонуңуз',
    'az': 'Şəkil formatı dəstəklənmir. JPG və ya PNG istifadə edin',
    'ka': 'სურათის ფორმატი არ არის მხარდაჭერილი. გამოიყენეთ JPG ან PNG',
  },
  'adm_load_courier_failed': {
    'en': 'Could not load courier data',
    'pt': 'Não foi possível carregar dados do courier',
    'fr': 'Impossible de charger les données du coursier',
    'ar': 'تعذر تحميل بيانات المندوب',
    'zh_Hans': '无法加载快递员数据',
    'tr': 'Kurye verileri yüklenemedi',
    'ur': 'کورئیر کا ڈیٹا لوڈ نہیں ہو سکا',
    'ru': 'Не удалось загрузить данные курьера',
    'ky': 'Курьер маалыматтары жүктөлгөн жок',
    'az': 'Kuryer məlumatları yüklənmədi',
    'ka': 'კურიერის მონაცემების ჩატვირთვა ვერ მოხერხდა',
  },
  'adm_save_account_failed': {
    'en': 'Could not save account data',
    'pt': 'Não foi possível salvar os dados da conta',
    'fr': 'Impossible d\'enregistrer les données du compte',
    'ar': 'تعذر حفظ بيانات الحساب',
    'zh_Hans': '无法保存账户数据',
    'tr': 'Hesap verileri kaydedilemedi',
    'ur': 'اکاؤنٹ کا ڈیٹا محفوظ نہیں ہو سکا',
    'ru': 'Не удалось сохранить данные аккаунта',
    'ky': 'Аккаунт маалыматтары сакталган жок',
    'az': 'Hesab məlumatları saxlanılmadı',
    'ka': 'ანგარიშის მონაცემების შენახვა ვერ მოხერხდა',
  },
  'adm_update_driver_status_failed': {
    'en': 'Could not update driver status',
    'pt': 'Could not update driver status',
    'fr': 'Could not update driver status',
    'ar': 'تعذر تحديث حالة السائق',
    'zh_Hans': '无法更新司机状态',
    'tr': 'Sürücü durumu güncellenemedi',
    'ur': 'ڈرائیور کی حیثیت اپ ڈیٹ نہیں ہو سکی',
    'ru': 'Не удалось обновить статус водителя',
    'ky': 'Айдоочу статусу жаңыланган жок',
    'az': 'Sürücü statusu yenilənmədi',
    'ka': 'მძღოლის სტატუსის განახლება ვერ მოხერხდა',
  },
  'adm_update_ticket_failed': {
    'en': 'Could not update ticket',
    'pt': 'Não foi possível atualizar o ticket',
    'fr': 'Impossible de mettre à jour le ticket',
    'ar': 'تعذر تحديث التذكرة',
    'zh_Hans': '无法更新工单',
    'tr': 'Bilet güncellenemedi',
    'ur': 'ٹکٹ اپ ڈیٹ نہیں ہو سکی',
    'ru': 'Не удалось обновить обращение',
    'ky': 'Тикет жаңыланган жок',
    'az': 'Bilet yenilənmədi',
    'ka': 'ბილეთის განახლება ვერ მოხერხდა',
  },
  'adm_update_city_status_failed': {
    'en': 'Could not update city status',
    'pt': 'Não foi possível atualizar o status da cidade',
    'fr': 'Impossible de mettre à jour le statut de la ville',
    'ar': 'تعذر تحديث حالة المدينة',
    'zh_Hans': '无法更新城市状态',
    'tr': 'Şehir durumu güncellenemedi',
    'ur': 'شہر کی حیثیت اپ ڈیٹ نہیں ہو سکی',
    'ru': 'Не удалось обновить статус города',
    'ky': 'Шаар статусу жаңыланган жок',
    'az': 'Şəhər statusu yenilənmədi',
    'ka': 'ქალაქის სტატუსის განახლება ვერ მოხერხდა',
  },
  'adm_save_data_failed': {
    'en': 'Could not save data',
    'pt': 'Não foi possível salvar os dados',
    'fr': 'Impossible d\'enregistrer les données',
    'ar': 'تعذر حفظ البيانات',
    'zh_Hans': '无法保存数据',
    'tr': 'Veri kaydedilemedi',
    'ur': 'ڈیٹا محفوظ نہیں ہو سکا',
    'ru': 'Не удалось сохранить данные',
    'ky': 'Маалымат сакталган жок',
    'az': 'Məlumat saxlanılmadı',
    'ka': 'მონაცემების შენახვა ვერ მოხერხდა',
  },
  'adm_password_update_failed': {
    'en': 'Could not update password',
    'pt': 'Não foi possível atualizar a senha',
    'fr': 'Impossible de mettre à jour le mot de passe',
    'ar': 'تعذر تحديث كلمة المرور',
    'zh_Hans': '无法更新密码',
    'tr': 'Şifre güncellenemedi',
    'ur': 'پاس ورڈ اپ ڈیٹ نہیں ہو سکا',
    'ru': 'Не удалось обновить пароль',
    'ky': 'Сырсөз жаңыланган жок',
    'az': 'Şifrə yenilənmədi',
    'ka': 'პაროლის განახლება ვერ მოხერხდა',
  },
  'adm_deactivate_failed': {
    'en': 'Could not deactivate',
    'pt': 'Não foi possível desativar',
    'fr': 'Impossible de désactiver',
    'ar': 'تعذر الإيقاف',
    'zh_Hans': '无法停用',
    'tr': 'Devre dışı bırakılamadı',
    'ur': 'غیر فعال نہیں ہو سکا',
    'ru': 'Не удалось деактивировать',
    'ky': 'Өчүрүлгөн жок',
    'az': 'Deaktiv edilmədi',
    'ka': 'გამორთვა ვერ მოხერხდა',
  },

  // Driver activation / review
  'adm_drv_reason_hint': {
    'en': 'Write a clear reason for the driver',
    'pt': 'Write a clear reason for the driver',
    'fr': 'Write a clear reason for the driver',
    'ar': 'اكتب السبب بوضوح للمندوب',
    'zh_Hans': '请为司机填写明确原因',
    'tr': 'Sürücü için net bir neden yazın',
    'ur': 'ڈرائیور کے لیے واضح وجہ لکھیں',
    'ru': 'Укажите понятную причину для водителя',
    'ky': 'Айдоочу үчүн так себепти жазыңыз',
    'az': 'Sürücü üçün aydın səbəb yazın',
    'ka': 'მძღოლისთვის მიუთითეთ მკაფიო მიზეზი',
  },
  'adm_drv_reject_title': {
    'en': 'Reject driver application',
    'pt': 'Recusar driver application',
    'fr': 'Refuser driver application',
    'ar': 'رفض طلب المندوب',
    'zh_Hans': '拒绝司机申请',
    'tr': 'Sürücü başvurusunu reddet',
    'ur': 'ڈرائیور کی درخواست مسترد کریں',
    'ru': 'Отклонить заявку водителя',
    'ky': 'Айдоочунун арызын четке кагуу',
    'az': 'Sürücü müraciətini rədd et',
    'ka': 'მძღოლის განაცხადის უარყოფა',
  },
  'adm_drv_request_changes_title': {
    'en': 'Request changes from driver',
    'pt': 'Solicitar alterações ao motorista',
    'fr': 'Demander des modifications au chauffeur',
    'ar': 'طلب تعديلات من المندوب',
    'zh_Hans': '要求司机修改资料',
    'tr': 'Sürücüden düzeltme iste',
    'ur': 'ڈرائیور سے ترامیم طلب کریں',
    'ru': 'Запросить правки у водителя',
    'ky': 'Айдоочудан оңдоолорду суроо',
    'az': 'Sürücüdən düzəliş istə',
    'ka': 'მძღოლისგან ცვლილებების მოთხოვნა',
  },
  'adm_drv_activated_title': {
    'en': 'Activated',
    'pt': 'Ativado',
    'fr': 'Activé',
    'ar': 'تم التفعيل',
    'zh_Hans': '已激活',
    'tr': 'Etkinleştirildi',
    'ur': 'فعال ہو گیا',
    'ru': 'Активировано',
    'ky': 'Иштетилди',
    'az': 'Aktivləşdirildi',
    'ka': 'გააქტიურებულია',
  },
  'adm_drv_activated_body': {
    'en': 'Driver activated successfully',
    'pt': 'Motorista activated successfully',
    'fr': 'Chauffeur activated successfully',
    'ar': 'تم تفعيل المندوب بنجاح',
    'zh_Hans': '司机已成功激活',
    'tr': 'Sürücü başarıyla etkinleştirildi',
    'ur': 'ڈرائیور کامیابی سے فعال ہو گیا',
    'ru': 'Водитель успешно активирован',
    'ky': 'Айдоочу ийгиликтүү иштетилди',
    'az': 'Sürücü uğurla aktivləşdirildi',
    'ka': 'მძღოლი წარმატებით გააქტიურდა',
  },
  'adm_drv_approve_confirm_title': {
    'en': 'Approve driver',
    'pt': 'Approve driver',
    'fr': 'Approve driver',
    'ar': 'اعتماد المندوب',
    'zh_Hans': '批准司机',
    'tr': 'Sürücüyü onayla',
    'ur': 'ڈرائیور منظور کریں',
    'ru': 'Одобрить водителя',
    'ky': 'Айдоочуну бекитүү',
    'az': 'Sürücünü təsdiqlə',
    'ka': 'მძღოლის დამტკიცება',
  },
  'adm_drv_approve_confirm_body': {
    'en': 'Approve registration for %s?',
    'pt': 'Approve registration for %s?',
    'fr': 'Approve registration for %s?',
    'ar': 'هل تريد اعتماد طلب تسجيل المندوب %s؟',
    'zh_Hans': '批准 %s 的注册申请？',
    'tr': '%s kaydını onaylamak istiyor musunuz?',
    'ur': 'کیا آپ %s کی رجسٹریشن منظور کرنا چاہتے ہیں؟',
    'ru': 'Одобрить заявку водителя %s?',
    'ky': '%s айдоочунун катталуусун бекитесизби?',
    'az': '%s sürücü qeydiyyatını təsdiqləmək istəyirsiniz?',
    'ka': 'დავამტკიცოთ მძღოლის %s რეგისტრაცია?',
  },
  'adm_drv_approved_title': {
    'en': 'Approved',
    'pt': 'Approved',
    'fr': 'Approved',
    'ar': 'تم الاعتماد',
    'zh_Hans': '已批准',
    'tr': 'Onaylandı',
    'ur': 'منظور ہو گیا',
    'ru': 'Одобрено',
    'ky': 'Бекитилди',
    'az': 'Təsdiqləndi',
    'ka': 'დამტკიცებულია',
  },
  'adm_drv_approved_body': {
    'en': 'Driver registration approved successfully.',
    'pt': 'Driver registration approved successfully.',
    'fr': 'Driver registration approved successfully.',
    'ar': 'تم اعتماد تسجيل المندوب بنجاح.',
    'zh_Hans': '司机注册已成功批准。',
    'tr': 'Sürücü kaydı başarıyla onaylandı.',
    'ur': 'ڈرائیور رجسٹریشن کامیابی سے منظور ہو گئی۔',
    'ru': 'Регистрация водителя успешно одобрена.',
    'ky': 'Айдоочунун катталуусу ийгиликтүү бекитилди.',
    'az': 'Sürücü qeydiyyatı uğurla təsdiqləndi.',
    'ka': 'მძღოლის რეგისტრაცია წარმატებით დამტკიცდა.',
  },
  'adm_drv_err_docs_not_approved': {
    'en': 'Driver documents still need review before approval.',
    'pt': 'Driver documents still need review before approval.',
    'fr': 'Driver documents still need review before approval.',
    'ar': 'ما زالت وثائق المندوب بحاجة لمراجعة قبل الاعتماد.',
    'zh_Hans': '批准前仍需审核司机证件。',
    'tr': 'Onaydan önce belgeler incelenmeli.',
    'ur': 'منظوری سے پہلے دستاویزات کا جائزہ درکار ہے۔',
    'ru': 'Перед одобрением нужно проверить документы.',
    'ky': 'Бекитүүдөн мурун документтерди текшерүү керек.',
    'az': 'Təsdiqdən əvvəl sənədlər yoxlanmalıdır.',
    'ka': 'დამტკიცებამდე დოკუმენტები უნდა შემოწმდეს.',
  },
  'adm_drv_err_email_not_verified': {
    'en': 'Driver email is not verified yet.',
    'pt': 'Driver email is not verified yet.',
    'fr': 'Driver email is not verified yet.',
    'ar': 'بريد المندوب غير مُوثَّق بعد.',
    'zh_Hans': '司机邮箱尚未验证。',
    'tr': 'Sürücü e-postası henüz doğrulanmadı.',
    'ur': 'ڈرائیور ای میل ابھی تصدیق شدہ نہیں۔',
    'ru': 'Email водителя ещё не подтверждён.',
    'ky': 'Айдоочунун emailы азырынча ырасталган эмес.',
    'az': 'Sürücü e-poçtu hələ təsdiqlənməyib.',
    'ka': 'მძღოლის ელფოსტა ჯერ არ არის დადასტურებული.',
  },
  'adm_drv_err_phone_required': {
    'en': 'Driver phone number is required.',
    'pt': 'Driver phone number is required.',
    'fr': 'Driver phone number is required.',
    'ar': 'رقم جوال المندوب مطلوب.',
    'zh_Hans': '需要司机手机号。',
    'tr': 'Sürücü telefon numarası gerekli.',
    'ur': 'ڈرائیور فون نمبر ضروری ہے۔',
    'ru': 'Нужен телефон водителя.',
    'ky': 'Айдоочунун телефону керек.',
    'az': 'Sürücü telefon nömrəsi tələb olunur.',
    'ka': 'მძღოლის ტელეფონი აუცილებელია.',
  },
  'adm_drv_err_stale': {
    'en': 'Application status changed. Refresh and try again.',
    'pt': 'Application status changed. Refresh and try again.',
    'fr': 'Application status changed. Refresh and try again.',
    'ar': 'حالة الطلب تغيّرت. حدّث الصفحة ثم أعد المحاولة.',
    'zh_Hans': '申请状态已变更。请刷新后重试。',
    'tr': 'Başvuru durumu değişti. Yenileyip tekrar deneyin.',
    'ur': 'درخواست کی حالت بدل گئی۔ تازہ کریں اور دوبارہ کوشش کریں۔',
    'ru': 'Статус заявки изменился. Обновите и повторите.',
    'ky': 'Арыздын статусу өзгөрдү. Жаңылап кайталап көрүңүз.',
    'az': 'Müraciət statusu dəyişdi. Yeniləyib yenidən cəhd edin.',
    'ka': 'განაცხადის სტატუსი შეიცვალა. განაახლეთ და სცადეთ თავიდან.',
  },
  'adm_drv_err_auth_missing': {
    'en': 'Driver auth account was not found.',
    'pt': 'Driver auth account was not found.',
    'fr': 'Driver auth account was not found.',
    'ar': 'حساب دخول المندوب غير موجود.',
    'zh_Hans': '未找到司机登录账户。',
    'tr': 'Sürücü oturum hesabı bulunamadı.',
    'ur': 'ڈرائیور لاگ اِن اکاؤنٹ نہیں ملا۔',
    'ru': 'Учётная запись водителя не найдена.',
    'ky': 'Айдоочунун кирүү аккаунту табылган жок.',
    'az': 'Sürücü giriş hesabı tapılmadı.',
    'ka': 'მძღოლის ავტორიზაციის ანგარიში ვერ მოიძებნა.',
  },
  'adm_drv_err_profile_incomplete': {
    'en': 'Driver profile is incomplete (city or vehicle type).',
    'pt': 'Driver profile is incomplete (city or vehicle type).',
    'fr': 'Driver profile is incomplete (city or vehicle type).',
    'ar': 'ملف المندوب غير مكتمل (المدينة أو نوع المركبة).',
    'zh_Hans': '司机资料不完整（城市或车型）。',
    'tr': 'Sürücü profili eksik (şehir veya araç tipi).',
    'ur': 'ڈرائیور پروفائل نامکمل ہے (شہر یا گاڑی کی قسم)۔',
    'ru': 'Профиль водителя неполный (город или тип ТС).',
    'ky': 'Айдоочунун профили толук эмес (шаар же унаа түрү).',
    'az': 'Sürücü profili natamamdır (şəhər və ya nəqliyyat tipi).',
    'ka': 'მძღოლის პროფილი არასრულია (ქალაქი ან ტრანსპორტის ტიპი).',
  },
  'adm_drv_err_approve_failed': {
    'en': 'Could not approve the driver. Please try again.',
    'pt': 'Could not approve the driver. Please try again.',
    'fr': 'Could not approve the driver. Please try again.',
    'ar': 'تعذر اعتماد المندوب. حاول مرة أخرى.',
    'zh_Hans': '无法批准司机。请重试。',
    'tr': 'Sürücü onaylanamadı. Lütfen tekrar deneyin.',
    'ur': 'ڈرائیور منظور نہیں ہو سکا۔ دوبارہ کوشش کریں۔',
    'ru': 'Не удалось одобрить водителя. Повторите попытку.',
    'ky': 'Айдоочу бекитилген жок. Кайра аракет кылыңыз.',
    'az': 'Sürücü təsdiqlənmədi. Yenidən cəhd edin.',
    'ka': 'მძღოლის დამტკიცება ვერ მოხერხდა. სცადეთ თავიდან.',
  },
  'adm_drv_request_changes_btn': {
    'en': 'Request changes',
    'pt': 'Solicitar alterações',
    'fr': 'Demander des modifications',
    'ar': 'طلب تعديلات',
    'zh_Hans': '要求修改',
    'tr': 'Düzeltme iste',
    'ur': 'ترامیم طلب کریں',
    'ru': 'Запросить правки',
    'ky': 'Оңдоолорду суроо',
    'az': 'Düzəliş istə',
    'ka': 'ცვლილებების მოთხოვნა',
  },
  'adm_drv_reject_btn': {
    'en': 'Reject application',
    'pt': 'Recusar application',
    'fr': 'Refuser application',
    'ar': 'رفض الطلب',
    'zh_Hans': '拒绝申请',
    'tr': 'Başvuruyu reddet',
    'ur': 'درخواست مسترد کریں',
    'ru': 'Отклонить заявку',
    'ky': 'Арызды четке кагуу',
    'az': 'Müraciəti rədd et',
    'ka': 'განაცხადის უარყოფა',
  },
  'adm_drv_select_work_city': {
    'en': 'Please select work city',
    'pt': 'Por favor select work city',
    'fr': 'Veuillez select work city',
    'ar': 'يرجى تحديد مدينة العمل',
    'zh_Hans': '请选择工作城市',
    'tr': 'Lütfen çalışma şehrini seçin',
    'ur': 'براہ کرم کام کا شہر منتخب کریں',
    'ru': 'Выберите город работы',
    'ky': 'Иштөө шаарын тандаңыз',
    'az': 'İş şəhərini seçin',
    'ka': 'აირჩიეთ სამუშაო ქალაქი',
  },
  'adm_drv_select_car_type': {
    'en': 'Please select vehicle type',
    'pt': 'Por favor select vehicle type',
    'fr': 'Veuillez select vehicle type',
    'ar': 'يرجى تحديد نوع السيارة',
    'zh_Hans': '请选择车型',
    'tr': 'Lütfen araç tipini seçin',
    'ur': 'براہ کرم گاڑی کی قسم منتخب کریں',
    'ru': 'Выберите тип автомобиля',
    'ky': 'Унаанын түрүн тандаңыз',
    'az': 'Avtomobil tipini seçin',
    'ka': 'აირჩიეთ ავტომობილის ტიპი',
  },
  'adm_drv_blocker_suspended': {
    'en': 'Account is suspended or blocked',
    'pt': 'Account is suspended or blocked',
    'fr': 'Account is suspended or blocked',
    'ar': 'الحساب موقوف أو محظور',
    'zh_Hans': '账户已停用或被封禁',
    'tr': 'Hesap askıya alınmış veya engellenmiş',
    'ur': 'اکاؤنٹ معطل یا بلاک ہے',
    'ru': 'Аккаунт приостановлен или заблокирован',
    'ky': 'Аккаунт токтотулган же бөгөттөлгөн',
    'az': 'Hesab dayandırılıb və ya bloklanıb',
    'ka': 'ანგარიში შეჩერებულია ან დაბლოკილია',
  },
  'adm_drv_blocker_work_area': {
    'en': 'Work area (city/village) is required',
    'pt': 'Work area (city/village) is required',
    'fr': 'Work area (city/village) is required',
    'ar': 'يجب تحديد منطقة العمل (القرية/المدينة)',
    'zh_Hans': '必须指定工作区域（城市/村庄）',
    'tr': 'Çalışma bölgesi (şehir/köy) zorunludur',
    'ur': 'کام کا علاقہ (شہر/گاؤں) ضروری ہے',
    'ru': 'Нужно указать район работы (город/село)',
    'ky': 'Иштөө аймагын (шаар/айыл) тандоо керек',
    'az': 'İş bölgəsi (şəhər/kənd) tələb olunur',
    'ka': 'სამუშაო ზონა (ქალაქი/სოფელი) აუცილებელია',
  },
  'adm_drv_blocker_vehicle_type': {
    'en': 'Vehicle type is required',
    'pt': 'O tipo de veículo é obrigatório',
    'fr': 'Le type de véhicule est requis',
    'ar': 'يجب تحديد نوع المركبة',
    'zh_Hans': '必须指定车型',
    'tr': 'Araç tipi zorunludur',
    'ur': 'گاڑی کی قسم ضروری ہے',
    'ru': 'Нужно указать тип транспорта',
    'ky': 'Унаанын түрүн тандоо керек',
    'az': 'Nəqliyyat tipi tələb olunur',
    'ka': 'ტრანსპორტის ტიპი აუცილებელია',
  },
  'adm_drv_blocker_photo': {
    'en': 'Profile photo is required',
    'pt': 'Perfil photo is required',
    'fr': 'Profil photo is required',
    'ar': 'صورة الملف الشخصي مطلوبة',
    'zh_Hans': '需要头像照片',
    'tr': 'Profil fotoğrafı zorunludur',
    'ur': 'پروفائل تصویر ضروری ہے',
    'ru': 'Нужна фотография профиля',
    'ky': 'Профиль сүрөтү керек',
    'az': 'Profil şəkli tələb olunur',
    'ka': 'პროფილის ფოტო აუცილებელია',
  },
  'adm_drv_blocker_id_doc': {
    'en': 'Identity document is required',
    'pt': 'Identity document is required',
    'fr': 'Identity document is required',
    'ar': 'مستند الهوية مطلوب',
    'zh_Hans': '需要身份证件',
    'tr': 'Kimlik belgesi zorunludur',
    'ur': 'شناختی دستاویز ضروری ہے',
    'ru': 'Нужен документ удостоверения личности',
    'ky': 'Жеке күбөлүк документи керек',
    'az': 'Şəxsiyyət sənədi tələb olunur',
    'ka': 'პირადობის დოკუმენტი აუცილებელია',
  },
  'adm_drv_blocker_open_changes': {
    'en': 'There are unresolved change requests',
    'pt': 'There are unresolved change requests',
    'fr': 'There are unresolved change requests',
    'ar': 'توجد طلبات تعديل مفتوحة',
    'zh_Hans': '仍有未处理的修改请求',
    'tr': 'Açık düzeltme talepleri var',
    'ur': 'غیر حل شدہ ترمیمی درخواستیں موجود ہیں',
    'ru': 'Есть незакрытые запросы на правки',
    'ky': 'Жабылбаган оңдоо сурамдары бар',
    'az': 'Açıq düzəliş sorğuları var',
    'ka': 'არის გადაუჭრელი ცვლილების მოთხოვნები',
  },
  'adm_drv_blocker_registration_not_approved': {
    'en':
        'Cannot activate until registration is approved. Use registration review.',
    'pt':
        'Cannot activate until registration is approved. Use registration review.',
    'fr':
        'Cannot activate until registration is approved. Use registration review.',
    'ar': 'لا يمكن تفعيل المندوب قبل اعتماد التسجيل. استخدم مراجعة التسجيل.',
    'zh_Hans': '注册批准前无法激活。请使用注册审核。',
    'tr': 'Kayıt onaylanmadan etkinleştirilemez. Kayıt incelemesini kullanın.',
    'ur':
        'رجسٹریشن منظور ہونے تک فعال نہیں کیا جا سکتا۔ رجسٹریشن جائزہ استعمال کریں۔',
    'ru':
        'Нельзя активировать до одобрения регистрации. Используйте проверку регистрации.',
    'ky':
        'Каттоо бекитилмейинче активдештирүүгө болбойт. Каттоо текшерүүсүн колдонуңуз.',
    'az':
        'Qeydiyyat təsdiqlənənə qədər aktivləşdirilə bilməz. Qeydiyyat yoxlamasından istifadə edin.',
    'ka':
        'რეგისტრაციის დამტკიცებამდე აქტივაცია შეუძლებელია. გამოიყენეთ რეგისტრაციის განხილვა.',
  },
  'adm_drv_deactivate_title': {
    'en': 'Deactivate driver account?',
    'pt': 'Deactivate driver account?',
    'fr': 'Deactivate driver account?',
    'ar': 'تعطيل تشغيل حساب المندوب؟',
    'zh_Hans': '停用司机账户？',
    'tr': 'Sürücü hesabı devre dışı bırakılsın mı?',
    'ur': 'ڈرائیور اکاؤنٹ غیر فعال کریں؟',
    'ru': 'Отключить аккаунт водителя?',
    'ky': 'Айдоочу аккаунтун өчүрөсүзбү?',
    'az': 'Sürücü hesabı deaktiv edilsin?',
    'ka': 'გავთიშოთ მძღოლის ანგარიში?',
  },
  'adm_drv_deactivate_body': {
    'en':
        'Driver will not receive bookings. Registration stays approved unless you suspend.',
    'pt':
        'Driver will not receive bookings. Registration stays approved unless you suspend.',
    'fr':
        'Driver will not receive bookings. Registration stays approved unless you suspend.',
    'ar':
        'لن يستقبل المندوب طلبات. يبقى التسجيل معتمداً ما لم يتم إيقاف الحساب.',
    'zh_Hans': '司机将无法接单。除非停权，注册状态保持已批准。',
    'tr': 'Sürücü iş alamaz. Askıya almazsanız kayıt onaylı kalır.',
    'ur': 'ڈرائیور کو بکنگ نہیں ملیں گی۔ معطل کیے بغیر رجسٹریشن منظور رہتی ہے۔',
    'ru':
        'Водитель не будет получать заказы. Регистрация остаётся одобренной, если не приостановить.',
    'ky': 'Айдоочу заказ албайт. Токтотпосоңуз каттоо бекитилген бойдон калат.',
    'az': 'Sürücü sifariş almayacaq. Dayandırmasanız qeydiyyat təsdiqli qalır.',
    'ka':
        'მძღოლი შეკვეთებს ვერ მიიღებს. რეგისტრაცია დამტკიცებული რჩება, თუ არ შეაჩერებთ.',
  },
  'adm_drv_deactivate_success': {
    'en': 'Driver deactivated',
    'pt': 'Driver deactivated',
    'fr': 'Driver deactivated',
    'ar': 'تم تعطيل تشغيل المندوب',
    'zh_Hans': '司机已停用',
    'tr': 'Sürücü devre dışı bırakıldı',
    'ur': 'ڈرائیور غیر فعال ہو گیا',
    'ru': 'Водитель отключён',
    'ky': 'Айдоочу өчүрүлдү',
    'az': 'Sürücü deaktiv edildi',
    'ka': 'მძღოლი გათიშულია',
  },
  'adm_drv_deactivate_action': {
    'en': 'Deactivate',
    'pt': 'Deactivate',
    'fr': 'Deactivate',
    'ar': 'تعطيل التشغيل',
    'zh_Hans': '停用',
    'tr': 'Devre dışı bırak',
    'ur': 'غیر فعال کریں',
    'ru': 'Отключить',
    'ky': 'Өчүрүү',
    'az': 'Deaktiv et',
    'ka': 'გათიშვა',
  },

  // Driver profile activate / suspend dialogs
  'adm_drv_suspend_title': {
    'en': 'Suspend driver',
    'pt': 'Suspender motorista',
    'fr': 'Suspendre le chauffeur',
    'ar': 'إيقاف المندوب',
    'zh_Hans': '停用司机',
    'tr': 'Sürücüyü askıya al',
    'ur': 'ڈرائیور معطل کریں',
    'ru': 'Приостановить водителя',
    'ky': 'Айдоочуну токтотуу',
    'az': 'Sürücünü dayandır',
    'ka': 'მძღოლის შეჩერება',
  },
  'adm_drv_suspend_body': {
    'en': 'Are you sure you want to suspend this driver?',
    'pt': 'Are you sure you want to suspend this driver?',
    'fr': 'Are you sure you want to suspend this driver?',
    'ar': 'هل أنت متأكد من إيقاف المندوب؟',
    'zh_Hans': '确定要停用此司机吗？',
    'tr': 'Bu sürücüyü askıya almak istediğinize emin misiniz?',
    'ur': 'کیا آپ واقعی اس ڈرائیور کو معطل کرنا چاہتے ہیں؟',
    'ru': 'Вы уверены, что хотите приостановить этого водителя?',
    'ky': 'Бул айдоочуну токтотууну чын эле каалайсызбы?',
    'az': 'Bu sürücünü dayandırmaq istədiyinizə əminsiniz?',
    'ka': 'ნამდვილად გსურთ ამ მძღოლის შეჩერება?',
  },
  'adm_drv_suspend_success': {
    'en': 'Driver suspended',
    'pt': 'Motorista suspended',
    'fr': 'Chauffeur suspended',
    'ar': 'تم إيقاف المندوب',
    'zh_Hans': '司机已停用',
    'tr': 'Sürücü askıya alındı',
    'ur': 'ڈرائیور معطل ہو گیا',
    'ru': 'Водитель приостановлен',
    'ky': 'Айдоочу токтотулду',
    'az': 'Sürücü dayandırıldı',
    'ka': 'მძღოლი შეჩერდა',
  },
  'adm_drv_activate_profile_title': {
    'en': 'Activate driver',
    'pt': 'Ativar motorista',
    'fr': 'Activer le chauffeur',
    'ar': 'تفعيل المندوب',
    'zh_Hans': '激活司机',
    'tr': 'Sürücüyü etkinleştir',
    'ur': 'ڈرائیور فعال کریں',
    'ru': 'Активировать водителя',
    'ky': 'Айдоочуну иштетүү',
    'az': 'Sürücünü aktivləşdir',
    'ka': 'მძღოლის გააქტიურება',
  },
  'adm_drv_activate_profile_body': {
    'en': 'Are you sure you want to activate this driver?',
    'pt': 'Are you sure you want to activate this driver?',
    'fr': 'Are you sure you want to activate this driver?',
    'ar': 'هل أنت متأكد من تفعيل المندوب؟',
    'zh_Hans': '确定要激活此司机吗？',
    'tr': 'Bu sürücüyü etkinleştirmek istediğinize emin misiniz?',
    'ur': 'کیا آپ واقعی اس ڈرائیور کو فعال کرنا چاہتے ہیں؟',
    'ru': 'Вы уверены, что хотите активировать этого водителя?',
    'ky': 'Бул айдоочуну иштетүүнү чын эле каалайсызбы?',
    'az': 'Bu sürücünü aktivləşdirmək istədiyinizə əminsiniz?',
    'ka': 'ნამდვილად გსურთ ამ მძღოლის გააქტიურება?',
  },

  // Login / password reset
  'adm_login_enter_email_first': {
    'en': 'Please enter your email first',
    'pt': 'Por favor enter your email first',
    'fr': 'Veuillez enter your email first',
    'ar': 'يرجى إدخال البريد الإلكتروني أولاً',
    'zh_Hans': '请先输入邮箱',
    'tr': 'Lütfen önce e-postanızı girin',
    'ur': 'پہلے اپنا ای میل درج کریں',
    'ru': 'Сначала введите электронную почту',
    'ky': 'Адегенде электрондук почтаңызды киргизиңиз',
    'az': 'Əvvəlcə e-poçtunuzu daxil edin',
    'ka': 'ჯერ შეიყვანეთ ელფოსტა',
  },
  'adm_login_reset_sent': {
    'en': 'Password reset link sent to your email',
    'pt': 'Password reset link sent to your email',
    'fr': 'Password reset link sent to your email',
    'ar': 'تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك',
    'zh_Hans': '密码重置链接已发送到您的邮箱',
    'tr': 'Şifre sıfırlama bağlantısı e-postanıza gönderildi',
    'ur': 'پاس ورڈ ری سیٹ لنک آپ کے ای میل پر بھیج دی گئی',
    'ru': 'Ссылка для сброса пароля отправлена на вашу почту',
    'ky': 'Сырсөздү калыбына келтирүү шилтемеси почтаңызга жөнөтүлдү',
    'az': 'Şifrə sıfırlama keçidi e-poçtunuza göndərildi',
    'ka': 'პაროლის აღდგენის ბმული გაიგზავნა ელფოსტაზე',
  },
  'adm_login_profile_failed': {
    'en': 'Could not load account data. Check your connection and try again.',
    'pt': 'Could not load account data. Check your connection and try again.',
    'fr': 'Could not load account data. Check your connection and try again.',
    'ar': 'تعذر تحميل بيانات الحساب. تحقق من الاتصال وحاول مرة أخرى.',
    'zh_Hans': '无法加载账户数据。请检查网络后重试。',
    'tr':
        'Hesap verileri yüklenemedi. Bağlantınızı kontrol edip tekrar deneyin.',
    'ur': 'اکاؤنٹ ڈیٹا لوڈ نہیں ہو سکا۔ کنکشن چیک کریں اور دوبارہ کوشش کریں۔',
    'ru':
        'Не удалось загрузить данные аккаунта. Проверьте соединение и повторите попытку.',
    'ky':
        'Аккаунт маалыматын жүктөө мүмкүн болбоду. Байланышты текшерип, кайра аракет кылыңыз.',
    'az':
        'Hesab məlumatları yüklənmədi. Bağlantını yoxlayıb yenidən cəhd edin.',
    'ka':
        'ანგარიშის მონაცემების ჩატვირთვა ვერ მოხერხდა. შეამოწმეთ კავშირი და სცადეთ ხელახლა.',
  },
  'adm_login_unauthorized': {
    'en': 'This account does not have permission to access the control panel.',
    'pt': 'This account does not have permission to access the control panel.',
    'fr': 'This account does not have permission to access the control panel.',
    'ar': 'هذا الحساب لا يملك صلاحية الدخول إلى لوحة التحكم',
    'zh_Hans': '此账户无权访问控制面板。',
    'tr': 'Bu hesabın kontrol paneline erişim yetkisi yok.',
    'ur': 'اس اکاؤنٹ کو کنٹرول پینل تک رسائی کی اجازت نہیں۔',
    'ru': 'У этой учётной записи нет доступа к панели управления.',
    'ky': 'Бул аккаунттун башкаруу панелине кирүүгө укугу жок.',
    'az': 'Bu hesabın idarəetmə panelinə girişi yoxdur.',
    'ka': 'ამ ანგარიშს მართვის პანელზე წვდომა არ აქვს.',
  },
  'adm_login_nav_failed': {
    'en': 'Could not open the control panel. Please try again.',
    'pt': 'Could not open the control panel. Por favor try again.',
    'fr': 'Could not open the control panel. Veuillez try again.',
    'ar': 'تعذر فتح لوحة التحكم. حاول مرة أخرى.',
    'zh_Hans': '无法打开控制面板。请重试。',
    'tr': 'Kontrol paneli açılamadı. Lütfen tekrar deneyin.',
    'ur': 'کنٹرول پینل نہیں کھل سکا۔ دوبارہ کوشش کریں۔',
    'ru': 'Не удалось открыть панель управления. Попробуйте ещё раз.',
    'ky': 'Башкаруу панелин ачуу мүмкүн болбоду. Кайра аракет кылыңыз.',
    'az': 'İdarəetmə paneli açıla bilmədi. Yenidən cəhd edin.',
    'ka': 'მართვის პანელის გახსნა ვერ მოხერხდა. სცადეთ ხელახლა.',
  },
  'adm_unauthorized_title': {
    'en': 'Unauthorized',
    'pt': 'Não autorizado',
    'fr': 'Non autorisé',
    'ar': 'غير مصرّح',
    'zh_Hans': '未授权',
    'tr': 'Yetkisiz',
    'ur': 'غیر مجاز',
    'ru': 'Нет доступа',
    'ky': 'Уруксат жок',
    'az': 'İcazəsiz',
    'ka': 'არაავტორიზებული',
  },
  'adm_login_user_not_found': {
    'en': 'No account found with this email.',
    'pt': 'Não account found with this email.',
    'fr': 'Non account found with this email.',
    'ar': 'لا يوجد حساب بهذا البريد الإلكتروني.',
    'zh_Hans': '未找到使用此邮箱的账户。',
    'tr': 'Bu e-posta ile kayıtlı hesap bulunamadı.',
    'ur': 'اس ای میل سے کوئی اکاؤنٹ نہیں ملا۔',
    'ru': 'Аккаунт с этой почтой не найден.',
    'ky': 'Бул почта менен аккаунт табылган жок.',
    'az': 'Bu e-poçt ilə hesab tapılmadı.',
    'ka': 'ამ ელფოსტით ანგარიში ვერ მოიძებნა.',
  },
  'adm_login_wrong_password': {
    'en': 'Incorrect email or password.',
    'pt': 'Incorrect email or password.',
    'fr': 'Incorrect email or password.',
    'ar': 'البريد الإلكتروني أو كلمة المرور غير صحيحة.',
    'zh_Hans': '邮箱或密码不正确。',
    'tr': 'E-posta veya şifre hatalı.',
    'ur': 'ای میل یا پاس ورڈ غلط ہے۔',
    'ru': 'Неверный email или пароль.',
    'ky': 'Электрондук почта же сырсөз туура эмес.',
    'az': 'E-poçt və ya şifrə yanlışdır.',
    'ka': 'ელფოსტა ან პაროლი არასწორია.',
  },
  'adm_login_invalid_email': {
    'en': 'Invalid email address.',
    'pt': 'Endereço de e-mail inválido.',
    'fr': 'Adresse e-mail invalide.',
    'ar': 'البريد الإلكتروني غير صالح.',
    'zh_Hans': '邮箱地址无效。',
    'tr': 'Geçersiz e-posta adresi.',
    'ur': 'غلط ای میل پتہ۔',
    'ru': 'Некорректный адрес электронной почты.',
    'ky': 'Электрондук почта дареги жараксыз.',
    'az': 'Yanlış e-poçt ünvanı.',
    'ka': 'არასწორი ელფოსტის მისამართი.',
  },
  'adm_login_user_disabled': {
    'en': 'This account has been disabled.',
    'pt': 'This account has been disabled.',
    'fr': 'This account has been disabled.',
    'ar': 'تم تعطيل هذا الحساب.',
    'zh_Hans': '此账户已被禁用。',
    'tr': 'Bu hesap devre dışı bırakıldı.',
    'ur': 'یہ اکاؤنٹ غیر فعال کر دیا گیا ہے۔',
    'ru': 'Этот аккаунт отключён.',
    'ky': 'Бул аккаунт өчүрүлгөн.',
    'az': 'Bu hesab deaktiv edilib.',
    'ka': 'ეს ანგარიში გამორთულია.',
  },
  'adm_login_too_many_requests': {
    'en': 'Too many attempts. Please try again later.',
    'pt': 'Too many attempts. Por favor try again later.',
    'fr': 'Too many attempts. Veuillez try again later.',
    'ar': 'محاولات كثيرة جداً. حاول لاحقاً.',
    'zh_Hans': '尝试次数过多。请稍后再试。',
    'tr': 'Çok fazla deneme. Lütfen daha sonra tekrar deneyin.',
    'ur': 'بہت زیادہ کوششیں۔ بعد میں دوبارہ کوشش کریں۔',
    'ru': 'Слишком много попыток. Попробуйте позже.',
    'ky': 'Өтө көп аракет. Кийинчерээк кайра аракет кылыңыз.',
    'az': 'Həddindən artıq cəhd. Daha sonra yenidən cəhd edin.',
    'ka': 'ძალიან ბევრი მცდელობა. მოგვიანებით სცადეთ.',
  },
  'adm_login_email_in_use': {
    'en': 'This email is already in use.',
    'pt': 'This email is already in use.',
    'fr': 'This email is already in use.',
    'ar': 'هذا البريد الإلكتروني مستخدم مسبقاً.',
    'zh_Hans': '此邮箱已被使用。',
    'tr': 'Bu e-posta zaten kullanılıyor.',
    'ur': 'یہ ای میل پہلے سے استعمال میں ہے۔',
    'ru': 'Этот email уже используется.',
    'ky': 'Бул электрондук почта мурунтан колдонулууда.',
    'az': 'Bu e-poçt artıq istifadə olunur.',
    'ka': 'ეს ელფოსტა უკვე გამოყენებულია.',
  },
  'adm_login_weak_password': {
    'en': 'Password is too weak.',
    'pt': 'A senha é muito fraca.',
    'fr': 'Le mot de passe est trop faible.',
    'ar': 'كلمة المرور ضعيفة جداً.',
    'zh_Hans': '密码太弱。',
    'tr': 'Şifre çok zayıf.',
    'ur': 'پاس ورڈ بہت کمزور ہے۔',
    'ru': 'Слишком слабый пароль.',
    'ky': 'Сырсөз өтө алсыз.',
    'az': 'Şifrə çox zəifdir.',
    'ka': 'პაროლი ძალიან სუსტია.',
  },

  // Generic user-facing errors
  'adm_err_permission': {
    'en': 'Permission denied.',
    'pt': 'Permissão negada.',
    'fr': 'Permission refusée.',
    'ar': 'ليس لديك صلاحية.',
    'zh_Hans': '没有权限。',
    'tr': 'İzin reddedildi.',
    'ur': 'اجازت نہیں۔',
    'ru': 'Нет доступа.',
    'ky': 'Уруксат жок.',
    'az': 'İcazə rədd edildi.',
    'ka': 'წვდომა აკრძალულია.',
  },
  'adm_err_not_found': {
    'en': 'Not found.',
    'pt': 'Nãot found.',
    'fr': 'Nont found.',
    'ar': 'غير موجود.',
    'zh_Hans': '未找到。',
    'tr': 'Bulunamadı.',
    'ur': 'نہیں ملا۔',
    'ru': 'Не найдено.',
    'ky': 'Табылган жок.',
    'az': 'Tapılmadı.',
    'ka': 'ვერ მოიძებნა.',
  },
  'adm_err_network': {
    'en': 'Could not connect. Please try again.',
    'pt': 'Could not connect. Please try again.',
    'fr': 'Could not connect. Please try again.',
    'ar': 'تعذر الاتصال، حاول مرة أخرى',
    'zh_Hans': '无法连接，请重试。',
    'tr': 'Bağlantı kurulamadı. Lütfen tekrar deneyin.',
    'ur': 'کنکشن نہیں ہو سکا، دوبارہ کوشش کریں۔',
    'ru': 'Не удалось подключиться. Попробуйте ещё раз.',
    'ky': 'Байланыш мүмкүн болбоду, кайра аракет кылыңыз.',
    'az': 'Qoşulmaq mümkün olmadı, yenidən cəhd edin.',
    'ka': 'დაკავშირება ვერ მოხერხდა, სცადეთ ხელახლა.',
  },
  'adm_err_unavailable': {
    'en': 'Service temporarily unavailable.',
    'pt': 'Service temporarily unavailable.',
    'fr': 'Service temporarily unavailable.',
    'ar': 'الخدمة غير متاحة مؤقتاً.',
    'zh_Hans': '服务暂时不可用。',
    'tr': 'Hizmet geçici olarak kullanılamıyor.',
    'ur': 'سروس عارضی طور پر دستیاب نہیں۔',
    'ru': 'Сервис временно недоступен.',
    'ky': 'Кызмат убактылуу жеткиликсиз.',
    'az': 'Xidmət müvəqqəti əlçatan deyil.',
    'ka': 'სერვისი დროებით მიუწვდომელია.',
  },
  'adm_err_generic': {
    'en': 'Something went wrong. Please try again.',
    'pt': 'Something went wrong. Por favor try again.',
    'fr': 'Something went wrong. Veuillez try again.',
    'ar': 'حدث خطأ. حاول مرة أخرى.',
    'zh_Hans': '出了点问题。请重试。',
    'tr': 'Bir şeyler ters gitti. Lütfen tekrar deneyin.',
    'ur': 'کچھ غلط ہو گیا۔ دوبارہ کوشش کریں۔',
    'ru': 'Что-то пошло не так. Попробуйте ещё раз.',
    'ky': 'Бир нерсе туура эмес болду. Кайра аракет кылыңыз.',
    'az': 'Bir şey səhv getdi. Yenidən cəhd edin.',
    'ka': 'რაღაც შეცდომა მოხდა. სცადეთ ხელახლა.',
  },
  'adm_err_query_index': {
    'en':
        'This list needs a database index. Retry shortly or contact support with the time of the error.',
    'pt':
        'This list needs a database index. Retry shortly or contact support with the time of the error.',
    'fr':
        'This list needs a database index. Retry shortly or contact support with the time of the error.',
    'ar':
        'هذه القائمة تحتاج فهرس قاعدة بيانات. أعد المحاولة بعد قليل أو تواصل مع الدعم مع وقت الخطأ.',
    'zh_Hans': '此列表需要数据库索引。请稍后重试，或联系支持并说明出错时间。',
    'tr':
        'Bu liste için bir veritabanı indeksi gerekli. Kısa süre sonra yeniden deneyin veya hata zamanıyla destekle iletişime geçin.',
    'ur':
        'اس فہرست کے لیے ڈیٹابیس انڈیکس درکار ہے۔ کچھ دیر بعد دوبارہ کوشش کریں یا سپورٹ سے رابطہ کریں۔',
    'ru':
        'Для этого списка нужен индекс базы данных. Повторите позже или обратитесь в поддержку, указав время ошибки.',
    'ky':
        'Бул тизме үчүн маалымат базасынын индекси керек. Бир аздан кийин кайталап көрүңүз же ката убактысы менен колдоого кайрылыңыз.',
    'az':
        'Bu siyahı üçün verilənlər bazası indeksi lazımdır. Bir az sonra yenidən cəhd edin və ya dəstəyə müraciət edin.',
    'ka':
        'ამ სიას სჭირდება მონაცემთა ბაზის ინდექსი. სცადეთ მოგვიანებით ან დაუკავშირდით მხარდაჭერას.',
  },
  'adm_err_already_exists': {
    'en': 'This record already exists.',
    'pt': 'Este registro já existe.',
    'fr': 'Cet enregistrement existe déjà.',
    'ar': 'هذا السجل موجود مسبقاً.',
    'zh_Hans': '该记录已存在。',
    'tr': 'Bu kayıt zaten mevcut.',
    'ur': 'یہ ریکارڈ پہلے سے موجود ہے۔',
    'ru': 'Такая запись уже существует.',
    'ky': 'Мындай жазуу мурунтан бар.',
    'az': 'Bu qeyd artıq mövcuddur.',
    'ka': 'ეს ჩანაწერი უკვე არსებობს.',
  },
  'adm_err_invalid_argument': {
    'en': 'Invalid data. Please check the form.',
    'pt': 'Invalid data. Por favor check the form.',
    'fr': 'Invalid data. Veuillez check the form.',
    'ar': 'بيانات غير صالحة. تحقق من النموذج.',
    'zh_Hans': '数据无效。请检查表单。',
    'tr': 'Geçersiz veri. Lütfen formu kontrol edin.',
    'ur': 'غلط ڈیٹا۔ فارم چیک کریں۔',
    'ru': 'Некорректные данные. Проверьте форму.',
    'ky': 'Жараксыз маалымат. Форманы текшериңиз.',
    'az': 'Yanlış məlumat. Formanı yoxlayın.',
    'ka': 'არასწორი მონაცემები. შეამოწმეთ ფორმა.',
  },
  'adm_err_unauthenticated': {
    'en': 'Please sign in before uploading.',
    'pt': 'Por favor sign in before uploading.',
    'fr': 'Veuillez sign in before uploading.',
    'ar': 'يجب تسجيل الدخول قبل رفع الصورة.',
    'zh_Hans': '请先登录再上传。',
    'tr': 'Yüklemeden önce giriş yapın.',
    'ur': 'اپ لوڈ سے پہلے سائن ان کریں۔',
    'ru': 'Войдите перед загрузкой.',
    'ky': 'Жүктөөдөн мурун кириңиз.',
    'az': 'Yükləməzdən əvvəl daxil olun.',
    'ka': 'ატვირთვამდე შედით სისტემაში.',
  },
  'adm_err_canceled': {
    'en': 'Upload canceled.',
    'pt': 'Upload cancelado.',
    'fr': 'Téléversement annulé.',
    'ar': 'تم إلغاء الرفع.',
    'zh_Hans': '上传已取消。',
    'tr': 'Yükleme iptal edildi.',
    'ur': 'اپ لوڈ منسوخ ہو گیا۔',
    'ru': 'Загрузка отменена.',
    'ky': 'Жүктөө жокко чыгарылды.',
    'az': 'Yükləmə ləğv edildi.',
    'ka': 'ატვირთვა გაუქმდა.',
  },
  'adm_err_empty_file': {
    'en': 'Could not read the image. Try another file.',
    'pt': 'Could not read the image. Try another file.',
    'fr': 'Could not read the image. Try another file.',
    'ar': 'لم يتم قراءة الصورة من المعرض. جرّب صورة أخرى.',
    'zh_Hans': '无法读取图片，请换一张。',
    'tr': 'Görsel okunamadı. Başka bir dosya deneyin.',
    'ur': 'تصویر نہیں پڑھی جا سکی۔ دوسری فائل آزمائیں۔',
    'ru': 'Не удалось прочитать изображение. Попробуйте другой файл.',
    'ky': 'Сүрөт окулган жок. Башка файлды байкап көрүңүз.',
    'az': 'Şəkil oxuna bilmədi. Başqa fayl sınayın.',
    'ka': 'სურათის წაკითხვა ვერ მოხერხდა. სცადეთ სხვა ფაილი.',
  },
  'adm_err_storage_quota': {
    'en':
        'Firebase Storage quota for this project is exceeded. Free this space in Firebase Console → Storage, or enable the Blaze plan. This is a project limit, not an app bug.',
    'pt':
        'Firebase Storage quota for this project is exceeded. Free this space in Firebase Console → Storage, or enable the Blaze plan. This is a project limit, not an app bug.',
    'fr':
        'Firebase Storage quota for this project is exceeded. Free this space in Firebase Console → Storage, or enable the Blaze plan. This is a project limit, not an app bug.',
    'ar':
        'تم تجاوز حصة Firebase Storage لهذا المشروع. حرّر مساحة من Console → Storage أو فعّل خطة Blaze. هذا حد على المشروع وليس خطأ في التطبيق.',
    'zh_Hans': '此项目的 Firebase Storage 配额已用尽。请在控制台清理或启用 Blaze 方案。这是项目限制，不是应用错误。',
    'tr':
        'Bu projenin Firebase Storage kotası aşıldı. Konsolda alan açın veya Blaze planını etkinleştirin. Bu bir uygulama hatası değil.',
    'ur':
        'اس پروجیکٹ کی Firebase Storage کوٹہ ختم ہو چکی ہے۔ Console سے جگہ خالی کریں یا Blaze آن کریں۔ یہ ایپ کا بگ نہیں۔',
    'ru':
        'Исчерпана квота Firebase Storage для проекта. Освободите место в Console или включите Blaze. Это лимит проекта, не ошибка приложения.',
    'ky':
        'Бул долбоордун Firebase Storage квотасы түгөндү. Console\'дон орун бошотуңуз же Blaze күйгүзүңүз. Бул колдонмо катасы эмес.',
    'az':
        'Bu layihənin Firebase Storage kvotası bitib. Console-da yer boşaldın və ya Blaze aktiv edin. Bu tətbiq xətası deyil.',
    'ka':
        'ამ პროექტის Firebase Storage კვოტა ამოიწურა. გაასუფთავეთ Console-ში ან ჩართეთ Blaze. ეს აპის შეცდომა არ არის.',
  },
  'adm_err_storage_billing': {
    'en':
        'Firebase Storage billing is not active. Enable Blaze (pay as you go) for this Firebase project, then try again.',
    'pt':
        'Firebase Storage billing is not active. Enable Blaze (pay as you go) for this Firebase project, then try again.',
    'fr':
        'Firebase Storage billing is not active. Enable Blaze (pay as you go) for this Firebase project, then try again.',
    'ar':
        'فوترة Firebase Storage غير مفعّلة. فعّل خطة Blaze لهذا المشروع ثم أعد المحاولة.',
    'zh_Hans': '未启用 Firebase Storage 计费。请为此项目启用 Blaze，然后重试。',
    'tr':
        'Firebase Storage faturalandırması aktif değil. Blaze planını etkinleştirip tekrar deneyin.',
    'ur': 'Firebase Storage بلنگ فعال نہیں۔ Blaze آن کر کے دوبارہ کوشش کریں۔',
    'ru':
        'Биллинг Firebase Storage не активен. Включите план Blaze и повторите попытку.',
    'ky':
        'Firebase Storage эсептөөсү өчүк. Blaze күйгүзүп кайра аракет кылыңыз.',
    'az':
        'Firebase Storage ödənişi aktiv deyil. Blaze-i aktiv edib yenidən cəhd edin.',
    'ka':
        'Firebase Storage ბილინგი არაა ჩართული. ჩართეთ Blaze და სცადეთ თავიდან.',
  },
  'adm_storage_fallback_saved': {
    'en':
        'Photo saved to your account (offline fallback). Cloud Storage quota/billing is blocking uploads — fix it in Firebase Console to use Storage URLs.',
    'pt':
        'Photo saved to your account (offline fallback). Cloud Storage quota/billing is blocking uploads — fix it in Firebase Console to use Storage URLs.',
    'fr':
        'Photo saved to your account (offline fallback). Cloud Storage quota/billing is blocking uploads — fix it in Firebase Console to use Storage URLs.',
    'ar':
        'تم حفظ الصورة في حسابك (وضع احتياطي). حصة/فوترة Firebase Storage تمنع الرفع السحابي — عالجها من Console لاستخدام روابط Storage.',
    'zh_Hans': '照片已保存到账户（备用方式）。云存储配额/计费阻止上传 — 请在控制台修复以使用 Storage 链接。',
    'tr':
        'Fotoğraf hesabınıza kaydedildi (yedek mod). Bulut kotası/fatura yüklemeyi engelliyor — Storage URL için Console’dan düzeltin.',
    'ur':
        'تصویر اکاؤنٹ میں محفوظ ہو گئی (fallback)۔ کلاؤڈ کوٹہ/بلنگ اپلوڈ روک رہی ہے — Console سے درست کریں۔',
    'ru':
        'Фото сохранено в аккаунте (резервный режим). Квота/биллинг Storage блокирует загрузку — исправьте в Console.',
    'ky':
        'Сүрөт аккаунтка сакталды (запас режим). Storage квотасы/эсептөө жүктөөнү бөгөттөп жатат — Console\'дон оңдоңуз.',
    'az':
        'Şəkil hesabınıza yazıldı (ehtiyat). Storage kvota/ödəniş yükləməni əngəlləyir — Console-da düzəldin.',
    'ka':
        'ფოტო ანგარიშში შეინახა (რეზერვი). Storage კვოტა/ბილინგი ბლოკავს ატვირთვას — გაასწორეთ Console-ში.',
  },

  // Car type form labels
  'adm_car_name_en': {
    'en': 'Car Name (English)',
    'pt': 'Nome do carro (inglês)',
    'fr': 'Nom de la voiture (anglais)',
    'ar': 'اسم السيارة (إنجليزي)',
    'zh_Hans': '车名（英语）',
    'tr': 'Araç adı (İngilizce)',
    'ur': 'گاڑی کا نام (انگریزی)',
    'ru': 'Название авто (английский)',
    'ky': 'Унаанын аталышы (англисче)',
    'az': 'Avtomobil adı (ingiliscə)',
    'ka': 'მანქანის სახელი (ინგლისური)',
  },
  'adm_car_name_ru': {
    'en': 'Car Name (Russian)',
    'pt': 'Nome do carro (russo)',
    'fr': 'Nom de la voiture (russe)',
    'ar': 'اسم السيارة (روسي)',
    'zh_Hans': '车名（俄语）',
    'tr': 'Araç adı (Rusça)',
    'ur': 'گاڑی کا نام (روسی)',
    'ru': 'Название авто (русский)',
    'ky': 'Унаанын аталышы (орусча)',
    'az': 'Avtomobil adı (rusca)',
    'ka': 'მანქანის სახელი (რუსული)',
  },
  'adm_car_name_ky': {
    'en': 'Car Name (Kyrgyz)',
    'pt': 'Nome do carro (quirguiz)',
    'fr': 'Nom de la voiture (kirghize)',
    'ar': 'اسم السيارة (قرغيزي)',
    'zh_Hans': '车名（吉尔吉斯语）',
    'tr': 'Araç adı (Kırgızca)',
    'ur': 'گاڑی کا نام (کرغیز)',
    'ru': 'Название авто (киргизский)',
    'ky': 'Унаанын аталышы (кыргызча)',
    'az': 'Avtomobil adı (qırğızca)',
    'ka': 'მანქანის სახელი (ყირგიზული)',
  },
  'adm_vehicle_code': {
    'en': 'Vehicle Code',
    'pt': 'Código do veículo',
    'fr': 'Code véhicule',
    'ar': 'رمز المركبة',
    'zh_Hans': '车辆代码',
    'tr': 'Araç kodu',
    'ur': 'گاڑی کا کوڈ',
    'ru': 'Код транспорта',
    'ky': 'Унаанын коду',
    'az': 'Nəqliyyat kodu',
    'ka': 'ტრანსპორტის კოდი',
  },
  'adm_car_name_ar': {
    'en': 'Car Name (Arabic)',
    'pt': 'Nome do carro (árabe)',
    'fr': 'Nom de la voiture (arabe)',
    'ar': 'اسم السيارة (عربي)',
    'zh_Hans': '车名（阿拉伯语）',
    'tr': 'Araç adı (Arapça)',
    'ur': 'گاڑی کا نام (عربی)',
    'ru': 'Название авто (арабский)',
    'ky': 'Унаанын аталышы (арабча)',
    'az': 'Avtomobil adı (ərəbcə)',
    'ka': 'მანქანის სახელი (არაბული)',
  },
  'adm_coords_example': {
    'en': 'Example: 24.713600, 46.675300',
    'pt': 'Example: 24.713600, 46.675300',
    'fr': 'Example: 24.713600, 46.675300',
    'ar': 'مثال: 24.713600, 46.675300',
    'zh_Hans': '示例：24.713600, 46.675300',
    'tr': 'Örnek: 24.713600, 46.675300',
    'ur': 'مثال: 24.713600, 46.675300',
    'ru': 'Пример: 24.713600, 46.675300',
    'ky': 'Мисал: 24.713600, 46.675300',
    'az': 'Nümunə: 24.713600, 46.675300',
    'ka': 'მაგალითი: 24.713600, 46.675300',
  },
  'adm_email_readonly_hint': {
    'en': 'Email cannot be changed here',
    'pt': 'Email cannot be changed here',
    'fr': 'Email cannot be changed here',
    'ar': 'لا يمكن تغيير البريد من هنا',
    'zh_Hans': '此处无法修改邮箱',
    'tr': 'E-posta buradan değiştirilemez',
    'ur': 'ای میل یہاں تبدیل نہیں ہو سکتا',
    'ru': 'Электронную почту здесь изменить нельзя',
    'ky': 'Электрондук почтаны бул жерден өзгөртүү мүмкүн эмес',
    'az': 'E-poçtu burada dəyişmək olmaz',
    'ka': 'ელფოსტის შეცვლა აქ შეუძლებელია',
  },
  'adm_self_deactivate_warn': {
    'en': 'Avoid deactivating your own account while using the panel',
    'pt': 'Avoid deactivating your own account while using the panel',
    'fr': 'Avoid deactivating your own account while using the panel',
    'ar': 'لا يُنصح بإيقاف حسابك أثناء استخدام اللوحة',
    'zh_Hans': '使用面板时请勿停用自己的账户',
    'tr': 'Paneli kullanırken kendi hesabınızı devre dışı bırakmayın',
    'ur': 'پینل استعمال کرتے ہوئے اپنا اکاؤنٹ غیر فعال نہ کریں',
    'ru': 'Не отключайте свой аккаунт, пока используете панель',
    'ky': 'Панелди колдонуп жатканда өз аккаунтуңузду өчүрбөңüz',
    'az': 'Paneldən istifadə edərkən öz hesabınızı deaktiv etməyin',
    'ka': 'პანელის გამოყენებისას საკუთარი ანგარიში ნუ გამორთოთ',
  },
  'adm_user_deleted_fmt': {
    'en': 'Deleted data for %s from the database',
    'pt': 'Excluird data for %s from the database',
    'fr': 'Supprimerd data for %s from the database',
    'ar': 'تم حذف بيانات %s من قاعدة البيانات',
    'zh_Hans': '已从数据库删除 %s 的数据',
    'tr': '%s verileri veritabanından silindi',
    'ur': 'ڈیٹا بیس سے %s کا ڈیٹا حذف کر دیا گیا',
    'ru': 'Данные %s удалены из базы',
    'ky': '%s маалыматтары маалымат базасынан өчүрүлдү',
    'az': '%s məlumatları bazadan silindi',
    'ka': '%s-ის მონაცემები წაიშალა ბაზიდან',
  },
  'adm_add_partner': {
    'en': 'Add partner',
    'pt': 'Adicionar partner',
    'fr': 'Ajouter partner',
    'ar': 'إضافة شريك',
    'zh_Hans': '添加合作伙伴',
    'tr': 'Ortak ekle',
    'ur': 'پارٹنر شامل کریں',
    'ru': 'Добавить партнёра',
    'ky': 'Өнөктөш кошуу',
    'az': 'Tərəfdaş əlavə et',
    'ka': 'პარტნიორის დამატება',
  },
  'adm_search_partners': {
    'en': 'Search partners...',
    'pt': 'Pesquisar partners...',
    'fr': 'Rechercher partners...',
    'ar': 'البحث في الشركاء...',
    'zh_Hans': '搜索合作伙伴...',
    'tr': 'Ortaklarda ara...',
    'ur': 'پارٹنرز تلاش کریں...',
    'ru': 'Поиск партнёров...',
    'ky': 'Өнөктөштөрдү издөө...',
    'az': 'Tərəfdaşlarda axtar...',
    'ka': 'პარტნიორების ძებნა...',
  },
  'adm_search_partners_hint': {
    'en': 'Search by partner name or description...',
    'pt': 'Pesquisar by partner name or description...',
    'fr': 'Rechercher by partner name or description...',
    'ar': 'ابحث باسم الشريك أو الوصف...',
    'zh_Hans': '按合作伙伴名称或描述搜索...',
    'tr': 'Ortak adı veya açıklama ile ara...',
    'ur': 'پارٹنر نام یا تفصیل سے تلاش کریں...',
    'ru': 'Поиск по имени или описанию партнёра...',
    'ky': 'Өнөктөштүн аталышы же сүрөттөмөсү боюнча издөө...',
    'az': 'Tərəfdaş adı və ya təsviri ilə axtarın...',
    'ka': 'მოძებნეთ პარტნიორის სახელით ან აღწერით...',
  },
  'adm_search_landmarks_hint': {
    'en': 'Search by name, description, or address...',
    'pt': 'Pesquisar by name, description, or address...',
    'fr': 'Rechercher by name, description, or address...',
    'ar': 'ابحث بالاسم أو الوصف أو العنوان...',
    'zh_Hans': '按名称、描述或地址搜索...',
    'tr': 'Ad, açıklama veya adres ile ara...',
    'ur': 'نام، تفصیل یا پتے سے تلاش کریں...',
    'ru': 'Поиск по имени, описанию или адресу...',
    'ky': 'Аталыш, сүрөттөмө же дарек боюнча издөө...',
    'az': 'Ad, təsvir və ya ünvan ilə axtarın...',
    'ka': 'მოძებნეთ სახელით, აღწერით ან მისამართით...',
  },
  'adm_company_manager_section': {
    'en': 'Company manager account (optional)',
    'pt': 'Company manager account (optional)',
    'fr': 'Company manager account (optional)',
    'ar': 'حساب مدير الشركة (اختياري)',
    'zh_Hans': '公司管理员账户（可选）',
    'tr': 'Şirket yöneticisi hesabı (isteğe bağlı)',
    'ur': 'کمپنی مینیجر اکاؤنٹ (اختیاری)',
    'ru': 'Аккаунт менеджера компании (необязательно)',
    'ky': 'Компания менеджеринин аккаунту (милдеттүү эмес)',
    'az': 'Şirkət meneceri hesabı (istəyə bağlı)',
    'ka': 'კომპანიის მენეჯერის ანგარიში (არასავალდებულო)',
  },
  'adm_company_manager_login_hint': {
    'en': 'For signing in and managing drivers later',
    'pt': 'For signing in and managing drivers later',
    'fr': 'For signing in and managing drivers later',
    'ar': 'للدخول وإدارة السائقين لاحقاً',
    'zh_Hans': '用于之后登录并管理司机',
    'tr': 'Daha sonra giriş yapıp sürücüleri yönetmek için',
    'ur': 'بعد میں سائن ان اور ڈرائیورز کے انتظام کے لیے',
    'ru': 'Для входа и управления водителями позже',
    'ky': 'Кийинчерээк кирүү жана айдоочуларды башкаруу үчүн',
    'az': 'Sonra daxil olmaq və sürücüləri idarə etmək üçün',
    'ka': 'შემდგომი შესვლისა და მძღოლების მართვისთვის',
  },
  'adm_partner_account_helper': {
    'en': 'A partner login is created when email and password are provided',
    'pt': 'A partner login is created when email and password are provided',
    'fr': 'A partner login is created when email and password are provided',
    'ar': 'يُنشأ حساب دخول للشريك عند إدخال البريد وكلمة المرور',
    'zh_Hans': '填写邮箱和密码后会创建合作伙伴登录账户',
    'tr': 'E-posta ve şifre girildiğinde ortak giriş hesabı oluşturulur',
    'ur': 'ای میل اور پاس ورڈ درج کرنے پر پارٹنر لاگ اِن بنتا ہے',
    'ru': 'Учётная запись партнёра создаётся при вводе почты и пароля',
    'ky': 'Почта жана сырсөз киргизилгенде өнөктөш аккаунту түзүлөт',
    'az': 'E-poçt və şifrə daxil ediləndə tərəfdaş girişi yaradılır',
    'ka': 'ელფოსტისა და პაროლის შეყვანისას იქმნება პარტნიორის ანგარიში',
  },
  'adm_bookings_count': {
    'en': '%s bookings',
    'pt': '%s reservas',
    'fr': '%s réservations',
    'ar': '%s حجز',
    'zh_Hans': '%s 笔预订',
    'tr': '%s rezervasyon',
    'ur': '%s بکنگ',
    'ru': '%s бронирований',
    'ky': '%s брондоо',
    'az': '%s bron',
    'ka': '%s ჯავშანი',
  },
  'adm_auth_error': {
    'en': 'Error: %s',
    'pt': 'Erro: %s',
    'fr': 'Erreur: %s',
    'ar': 'خطأ: %s',
    'zh_Hans': '错误：%s',
    'tr': 'Hata: %s',
    'ur': 'خرابی: %s',
    'ru': 'Ошибка: %s',
    'ky': 'Ката: %s',
    'az': 'Xəta: %s',
    'ka': 'შეცდომა: %s',
  },
  'adm_auth_recent_login': {
    'en':
        'Too long since most recent sign in. Sign in again before updating your email.',
    'pt':
        'Too long since most recent sign in. Sign in again before updating your email.',
    'fr':
        'Too long since most recent sign in. Sign in again before updating your email.',
    'ar':
        'مر وقت طويل منذ آخر تسجيل دخول. سجّل الدخول مجدداً قبل تحديث البريد.',
    'zh_Hans': '距离上次登录过久。请重新登录后再更新邮箱。',
    'tr':
        'Son oturumun üzerinden çok zaman geçti. E-postayı güncellemeden önce yeniden giriş yapın.',
    'ur':
        'آخری سائن اِن کے بعد کافی وقت ہو گیا۔ ای میل اپ ڈیٹ سے پہلے دوبارہ سائن اِن کریں۔',
    'ru': 'Слишком давно был вход. Войдите снова перед обновлением почты.',
    'ky':
        'Акыркы кирүүдөн көп убакыт өттү. Почтаны жаңылоодон мурун кайра кириңиз.',
    'az':
        'Son girişdən çox vaxt keçib. E-poçtu yeniləməzdən əvvəl yenidən daxil olun.',
    'ka':
        'ბოლო შესვლიდან დიდი დრო გავიდა. ელფოსტის განახლებამდე თავიდან შეხვიდეთ.',
  },
  'adm_choose_source': {
    'en': 'Choose Source',
    'pt': 'Escolher origem',
    'fr': 'Choisir la source',
    'ar': 'اختر المصدر',
    'zh_Hans': '选择来源',
    'tr': 'Kaynak seçin',
    'ur': 'ذریعہ منتخب کریں',
    'ru': 'Выберите источник',
    'ky': 'Булагын тандаңыз',
    'az': 'Mənbə seçin',
    'ka': 'აირჩიეთ წყარო',
  },
  'adm_gallery': {
    'en': 'Gallery',
    'pt': 'Galeria',
    'fr': 'Galerie',
    'ar': 'معرض الصور',
    'zh_Hans': '图库',
    'tr': 'Galeri',
    'ur': 'گیلری',
    'ru': 'Галерея',
    'ky': 'Галерея',
    'az': 'Qalereya',
    'ka': 'გალერეა',
  },
  'adm_gallery_photo': {
    'en': 'Gallery (Photo)',
    'pt': 'Galeria (Foto)',
    'fr': 'Galerie (Photo)',
    'ar': 'معرض الصور',
    'zh_Hans': '图库（照片）',
    'tr': 'Galeri (Fotoğraf)',
    'ur': 'گیلری (تصویر)',
    'ru': 'Галерея (фото)',
    'ky': 'Галерея (сүрөт)',
    'az': 'Qalereya (Foto)',
    'ka': 'გალერეა (ფოტო)',
  },
  'adm_gallery_video': {
    'en': 'Gallery (Video)',
    'pt': 'Galeria (Vídeo)',
    'fr': 'Galerie (Vidéo)',
    'ar': 'معرض الفيديو',
    'zh_Hans': '图库（视频）',
    'tr': 'Galeri (Video)',
    'ur': 'گیلری (ویڈیو)',
    'ru': 'Галерея (видео)',
    'ky': 'Галерея (видео)',
    'az': 'Qalereya (Video)',
    'ka': 'გალერეა (ვიდეო)',
  },
};

/// Replaces `%s` placeholders in localized templates.
String formatAdminTranslation(String template, Object value) =>
    template.replaceAll('%s', value.toString());
