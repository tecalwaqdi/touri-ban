...
Future<List<SelectedFile>?> selectMedia({
  ...
  final pathName = pickedMedia!.name.isNotEmpty
      ? pickedMedia.name
      : pickedMedia.path.split('/').last;
  final path = _getStoragePath(storageFolderPath, pathName, isVideo);
  final dimensions = includeDimensions
      ? isVideo
...
bool validateFileFormat(String filePath, BuildContext context) {
  ...
  if (type != null && allowedFormats.contains(type)) {
    return true;
  }
...
String _getStoragePath(
  ...
  final ext = _resolveFileExtension(filePath, isVideo);
  final indexStr = index != null ? '_$index' : '';
  return '$pathPrefix/$timestamp$indexStr.$ext';
}

String _resolveFileExtension(String filePath, bool isVideo) {
  if (isVideo) {
    return 'mp4';
  }
