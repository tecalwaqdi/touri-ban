...
class FirebaseAuthManager extends AuthManager
  ...
  ///
  /// [ensureUserDoc]: when false, skips [maybeCreateUser] so callers
  /// (e.g. driver registration) can create the full profile in one write.
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      if (userCredential?.user != null && ensureUserDoc) {
        await maybeCreateUser(userCredential!.user!);
      }
      return userCredential == null
          ? null
          : MndobFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      DriverAuthErrors.logSafely(e);
      final errorMsg = DriverAuthErrors.localized(context, e);
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
